...
def test_insufficient_energy_disabled_and_no_request_before_enabled():
    ...
    assert 'if (!primary.enabled || !actionType)' in execute
    assert execute.index('if (!primary.enabled || !actionType)') < execute.index('/api/polywar/action')
...
def test_pan_greater_than_threshold_sends_zero_actions():
    assert "dist > 8" in JS
    assert "this.drag.pan = true" in JS
    assert "!drag.pan" in JS
...
def test_secondary_more_menu_contextual_actions():
    assert "secondaryActions(c, primary)" in JS
