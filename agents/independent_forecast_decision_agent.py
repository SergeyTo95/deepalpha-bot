...
class IndependentForecastDecisionAgent(SafeDecisionAgent):
    """Require a point forecast that is distinct from simple market repetition.

    The normal DecisionAgent already receives market prices and available evidence.
    This runtime layer clarifies the product contract for providers such as Kimi:
    DeepAlpha must always return a numeric point estimate, while weak or missing
    evidence lowers confidence rather than deleting the forecast.
    """
    ...
    def _build_prompt(self, *args: Any, **kwargs: Any) -> str:
        prompt = super()._build_prompt(*args, **kwargs)
        lang = str(kwargs.get("lang") or "ru")
        return prompt + self.independent_probability_rules(lang)
