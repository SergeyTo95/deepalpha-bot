...
def _created_today(value: Any) -> bool:
    if isinstance(value, datetime):
        value = value.date()
    if isinstance(value, date):
        return value == attachment_service._utcnow().date()
    return False
...
def _enforce_daily_quota(
    ...
    *,
    ...
    if current_bytes + int(added_bytes) > daily_bytes_limit:
        raise attachment_service.AttachmentError(
    ...
    try:
        ...
        if existing:
            ...
            if existing_status == "failed" and existing_deleted_at is None:
                ...
                # start a second worker that could race completion/recovery.
                raise attachment_service.AttachmentError(
                    "attachment_upload_in_progress",
                    status=409,
                )

            # Rows already created today were counted by the original quota
            # reservation. Reviving an older row moves it into today's ledger,
            # so serialize and reapply today's count/byte limits first.
            if not _created_today(existing_created_at):
...
def scrub_unlinked_known_attachment(
    ...
    """Scrub a non-ready reservation while preserving recoverable ready rows."""
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            ...
            if str(row[0] or "") == "ready":
                # A ready deterministic row is the recovery record for every
                # request that used this key. Never invalidate an ID another
                # concurrent retry may already have received.
                conn.rollback()
                return False
            cursor.execute(
                """
...
def _reconcile_completion_error(
    *,
    attachment_id: str,
    user_id: int,
    conversation_id: str,
    ...
    """Recover a committed ready row or scrub a rolled-back reservation.

    The same advisory lock used by retries prevents cleanup from invalidating a
    ready attachment already returned by a concurrent request.
    """
    attempts = upload_service._env_int(
        "VELIA_ATTACHMENTS_SCRUB_ATTEMPTS",
        3,
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            ...
            if not row:
                ...
                return None
            if (
                str(row[1] or "") != str(conversation_id)
                or int(row[5] or 0) != int(expected_size)
                or (str(row[6] or "") and str(row[6]) != str(expected_digest))
            ):
                conn.rollback()
                return None
            if str(row[9] or "") == "ready" and row[11] is None:
                recovered = _serialize_existing(row)
                conn.commit()
                return recovered
...
def create_attachment_idempotently(
    ...
    *,
    ...
    try:
        ...
        recovered = _reconcile_completion_error(
            attachment_id=attachment_id,
            user_id=int(user_id),
            conversation_id=str(conversation_id),
