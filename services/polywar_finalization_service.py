...
def finalize_season(conn,season_id:int,victory_type='time',winner_faction_id=None,now=None):
    ...
    polywar._execute(c,"UPDATE polywar_seasons SET status='finalizing',victory_type=%s,winner_faction_id=%s,finalization_started_at=%s WHERE id=%s AND status!='completed'",(victory_type,winner_faction_id,now,season_id))
    stats=polywar._fetchall(c,'SELECT s.*,f.is_system FROM polywar_faction_season_stats s JOIN polywar_factions f ON f.id=s.faction_id WHERE s.season_id=%s',(season_id,))
    ...
    rows=[]
    for s in stats:
        fid=int(s['faction_id']); is_system=1 if fid==NULL_STATE_FACTION_ID or int(s.get('is_system') or 0) else 0; rank=None if is_system else ranks.get(fid)
        snap={'faction_id':fid,'rank':rank,'is_system':is_system}
        polywar._execute(c,'INSERT INTO polywar_season_results (season_id,faction_id,rank,is_system,influence_score,controlled_cells_count,controlled_sectors_count,controlled_capitals_count,active_members_count,total_faction_contribution,is_winner,snapshot_json,created_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT (season_id,faction_id) DO NOTHING',(season_id,fid,rank,is_system,s.get('influence_score') or 0,s.get('controlled_cells_count') or 0,s.get('controlled_sectors_count') or 0,s.get('controlled_capitals_count') or 0,s.get('active_members_count') or 0,contrib.get(fid) or 0,1 if fid==winner_faction_id else 0,json.dumps(snap,sort_keys=True),now)); rows.append(snap)
    rewards=_calculate_rewards(conn,season_id,ranks,winner_faction_id)
    snapshot={'season_id':season_id,'victory_type':victory_type,'winner_faction_id':winner_faction_id,'factions':rows,'rewards':rewards,'completed_at':polywar._iso(now),'version':1}
    h=hashlib.sha256(json.dumps(snapshot,sort_keys=True,separators=(',',':')).encode()).hexdigest()
...
def claim_reward(user_id:int,season_id:int,idempotency_key:str):
    ...
    try:
        init_finalization_schema(conn); r=polywar._fetchone(c,'SELECT * FROM polywar_player_season_rewards WHERE season_id=%s AND user_id=%s',(season_id,user_id))
        if not r: raise ValueError('reward_not_found')
        if r['status']=='claimed': return {'ok':True,'claimed':True,'duplicate':True,'reward':r}
        if int(r.get('total_reward') or 0)<=0 or r['status']=='ineligible': raise ValueError('reward_ineligible')
        now=_now(); polywar._execute(c,"UPDATE polywar_player_season_rewards SET status='processing',claim_started_at=%s WHERE season_id=%s AND user_id=%s",(now,season_id,user_id)); conn.commit()
