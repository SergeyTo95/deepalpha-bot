...
def ensure_squad_season_config(conn, season_id:int, *, existing_active:Optional[bool]=None):
    ...
    _execute(c,"""INSERT INTO polywar_squad_season_config (season_id,enabled,config_version,spawn_interval_minutes,move_interval_minutes,max_active_per_faction,ttl_minutes,max_hp,supply_distance,pressure_ttl_minutes,neutral_pressure_per_step,enemy_pressure_per_step,enemy_pressure_cap,capital_pressure_cap,combat_damage_per_tick,support_energy_cost,support_hp,max_catchup_ticks,created_at,updated_at) VALUES (%s,%s,1,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""", (season_id, vals['enabled'], vals['spawn_interval_minutes'], vals['move_interval_minutes'], vals['max_active_per_faction'], vals['ttl_minutes'], vals['max_hp'], vals['supply_distance'], vals['pressure_ttl_minutes'], vals['neutral_pressure_per_step'], vals['enemy_pressure_per_step'], vals['enemy_pressure_cap'], vals['capital_pressure_cap'], vals['combat_damage_per_tick'], vals['support_energy_cost'], vals['support_hp'], vals['max_catchup_ticks'], now, now))
    logger.info('polywar_squad_season_config_created season_id=%s enabled=%s', season_id, vals['enabled'])
    return _fetchone(c,'SELECT * FROM polywar_squad_season_config WHERE season_id=%s',(season_id,))
...
def spawn_due_squads_in_transaction(conn, season_id:int, now=None):
    now=now or _now(); cfg=ensure_squad_season_config(conn, season_id)
    if not int(cfg['enabled']): return 0
    c=conn.cursor(); total=int((_fetchone(c,"SELECT COUNT(*) AS n FROM polywar_faction_squads WHERE season_id=%s AND status IN ('spawning','marching','engaged','waiting_for_supply','waiting_for_players','retreating')",(season_id,)) or {}).get('n') or 0)
    if total>=HARD_ACTIVE_CAP: return 0
    season=_fetchone(c,'SELECT * FROM polywar_seasons WHERE id=%s',(season_id,)); seed=season.get('secret_seed','seed'); config=m.load_map_config(conn, season_id=season_id); spawned=0
    ...
    for f in factions:
        ...
        fid=int(f['id']); members=int((_fetchone(c,'SELECT COUNT(*) AS n FROM polywar_players WHERE season_id=%s AND faction_id=%s',(season_id,fid)) or {}).get('n') or 0)
        if members<=0: continue
        active=int((_fetchone(c,"SELECT COUNT(*) AS n FROM polywar_faction_squads WHERE season_id=%s AND faction_id=%s AND status IN ('spawning','marching','engaged','waiting_for_supply','waiting_for_players','retreating')",(season_id,fid)) or {}).get('n') or 0)
        ...
        if last.get('t') and str(last.get('t')) > _iso(now - timedelta(minutes=int(cfg['spawn_interval_minutes']))): continue
...
def process_squad_tick_in_transaction(conn, season_id:int, now=None):
    now=now or _now(); cfg=ensure_squad_season_config(conn, season_id); c=conn.cursor()
    tick=int(now.timestamp() // max(60, int(cfg['move_interval_minutes'])*60))
    try: _execute(c,"INSERT INTO polywar_squad_ticks (season_id,tick_index,scheduled_at,started_at,status,created_at) VALUES (%s,%s,%s,%s,'processing',%s)",(season_id,tick,now,now,now))
    except Exception: return {'spawned_count':0,'moved_count':0,'combat_count':0,'pressure_count':0,'duplicate':True}
    logger.info('polywar_squad_tick_started season_id=%s tick_index=%s',season_id,tick)
    spawned=spawn_due_squads_in_transaction(conn,season_id,now) if int(cfg['enabled']) else 0; moved=combat=pressure=0; processed=[]
    season=_fetchone(c,'SELECT * FROM polywar_seasons WHERE id=%s',(season_id,)); seed=season.get('secret_seed','seed'); config=m.load_map_config(conn, season_id=season_id)
    if int(cfg['enabled']):
        due=_fetchall(c,"SELECT * FROM polywar_faction_squads WHERE season_id=%s AND status IN ('spawning','marching','engaged','waiting_for_supply','waiting_for_players','retreating') AND next_move_at<=%s ORDER BY id LIMIT %s",(season_id,now,HARD_ACTIVE_CAP))
        for s in due:
            processed.append(int(s['id']))
            if _as_dt(s['expires_at']) <= now: _execute(c,"UPDATE polywar_faction_squads SET status='expired',updated_at=%s WHERE id=%s",(now,s['id'])); continue
            if int(s['hp'])<=0: _execute(c,"UPDATE polywar_faction_squads SET status='destroyed',updated_at=%s WHERE id=%s",(now,s['id'])); continue
            if s['status']=='engaged' and s.get('engaged_squad_id'):
                other=_fetchone(c,"SELECT * FROM polywar_faction_squads WHERE id=%s AND status='engaged'",(s['engaged_squad_id'],))
                if other:
                    hp=max(0,int(s['hp'])-int(cfg['combat_damage_per_tick'])); ohp=max(0,int(other['hp'])-int(cfg['combat_damage_per_tick']))
                    _execute(c,"UPDATE polywar_faction_squads SET hp=%s,status=%s,next_move_at=%s,updated_at=%s WHERE id=%s",(hp,'destroyed' if hp<=0 else 'engaged',now+timedelta(minutes=int(cfg['move_interval_minutes'])),now,s['id']))
                    _execute(c,"UPDATE polywar_faction_squads SET hp=%s,status=%s,next_move_at=%s,updated_at=%s WHERE id=%s",(ohp,'destroyed' if ohp<=0 else 'engaged',now+timedelta(minutes=int(cfg['move_interval_minutes'])),now,other['id']))
                    combat+=1; continue
            ...
            if kind=='engage':
                o=val; _execute(c,"UPDATE polywar_faction_squads SET status='engaged',engaged_squad_id=%s,next_move_at=%s,updated_at=%s WHERE id=%s",(o['id'],now+timedelta(minutes=int(cfg['move_interval_minutes'])),now,s['id'])); _execute(c,"UPDATE polywar_faction_squads SET status='engaged',engaged_squad_id=%s,next_move_at=%s,updated_at=%s WHERE id=%s",(s['id'],now+timedelta(minutes=int(cfg['move_interval_minutes'])),now,o['id'])); logger.info('polywar_squad_engaged season_id=%s squad_id=%s enemy_squad_id=%s',season_id,s['id'],o['id']); combat+=1; continue
            if kind=='move':
                nx,ny=val; _execute(c,"UPDATE polywar_faction_squads SET previous_x=x,previous_y=y,x=%s,y=%s,status='marching',move_index=move_index+1,blocked_ticks=0,last_moved_at=%s,next_move_at=%s,updated_at=%s WHERE id=%s",(nx,ny,now,now+timedelta(minutes=int(cfg['move_interval_minutes'])),now,s['id'])); ns=dict(s); ns.update({'x':nx,'y':ny}); pressure+=_apply_pressure(conn,ns,cfg,now,config); moved+=1; logger.info('polywar_squad_moved season_id=%s squad_id=%s x=%s y=%s',season_id,s['id'],nx,ny)
            else:
    ...
    outcome={'processed_squad_ids':processed,'expired_pressure':cleaned}
    ...
    return {'spawned_count':spawned,'moved_count':moved,'combat_count':combat,'pressure_count':pressure}

def ensure_squads_caught_up_in_transaction(conn, season_id:int, now=None):
    cfg=ensure_squad_season_config(conn, season_id); total={'spawned_count':0,'moved_count':0,'combat_count':0,'pressure_count':0}
    for _ in range(int(cfg['max_catchup_ticks'])):
        out=process_squad_tick_in_transaction(conn,season_id,now or _now())
        for k in total: total[k]+=int(out.get(k) or 0)
        break
    return total
...
def visible_squads(user_id:int, min_x:int, min_y:int, max_x:int, max_y:int):
    ...
    try:
        ...
        c=conn.cursor(); squads=_fetchall(c,"SELECT id,faction_id,x,y,previous_x,previous_y,hp,max_hp,status,target_x,target_y,next_move_at,expires_at FROM polywar_faction_squads WHERE season_id=%s AND x BETWEEN %s AND %s AND y BETWEEN %s AND %s AND status IN ('spawning','marching','engaged','waiting_for_supply','waiting_for_players','retreating') ORDER BY id LIMIT 200",(sid,min_x,max_x,min_y,max_y))
        ...
        return {'ok':True,'season_id':sid,'server_timestamp':int(time.time()),'squads':[{**dict(r),'next_move_at':_iso(r.get('next_move_at')),'expires_at':_iso(r.get('expires_at'))} for r in squads],'pressure':[{**dict(r),'expires_at':_iso(r.get('expires_at'))} for r in pressure]}
    finally: conn.close()

def support_squad(user_id:int, squad_id:int, idempotency_key:str):
    ...
    try:
        polywar.init_polywar_schema(conn); init_squad_schema(conn); conn.commit();
        if _is_sqlite(conn): c.execute('BEGIN IMMEDIATE')
        else: _execute(c,'BEGIN')
        season=m.get_active_season_readonly(conn); sid=int(season['id']); cfg=ensure_squad_season_config(conn,sid); player=polywar.get_or_create_player(user_id,sid,conn); fid=player.get('faction_id')
        ...
        now=_now(); cost=int(cfg['support_energy_cost']); _,_,energy=mines.spend_player_energy(conn,player,cost,now); hp=min(int(sq['max_hp']), int(sq['hp'])+int(cfg['support_hp']))
        ...
        _execute(c,"INSERT INTO polywar_events (season_id,user_id,faction_id,event_type,message,created_at) VALUES (%s,%s,%s,'squad_supported',%s,%s)",(sid,user_id,fid,f'Squad supported at {sq["x"]},{sq["y"]}',now))
        payload={'ok':True,'outcome':'squad_supported','squad':{'id':squad_id,'hp':hp,'max_hp':int(sq['max_hp'])},'energy':energy}
        mines.insert_outcome(conn,sid,user_id,idempotency_key,'support_squad',int(sq['x']),int(sq['y']),'squad_supported',cost,payload,now)
