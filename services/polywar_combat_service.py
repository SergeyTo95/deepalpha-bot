...
logger=logging.getLogger(__name__); _RATE=defaultdict(deque)
...
def _rate(uid):
    ...
    if len(q)>=20: raise ValueError('rate_limited')
    q.append(now)
...
def _materialize(conn,sid,x,y,owner,now):
    ...
    c=conn.cursor()
    if polywar._is_sqlite(conn): polywar._execute(c,'INSERT OR IGNORE INTO polywar_cells (season_id,x,y,owner_faction_id,capture_progress,updated_at) VALUES (%s,%s,%s,%s,100,%s)',(sid,x,y,owner,now))
    else: polywar._execute(c,'INSERT INTO polywar_cells (season_id,x,y,owner_faction_id,capture_progress,updated_at) VALUES (%s,%s,%s,%s,100,%s) ON CONFLICT (season_id,x,y) DO NOTHING',(sid,x,y,owner,now))
...
def combat_action(user_id:int, action_type:str, x:int, y:int, idempotency_key:str):
    ...
    try:
        ...
        dup=_dup(conn,sid,user_id,idempotency_key)
        if dup: return dup
        _rate(user_id)
        _begin(conn,c)
        dup=_dup(conn,sid,user_id,idempotency_key)
        ...
        if action_type=='attack':
            ...
            if outcome=='territory_captured':
                ...
                contested_at=now if new_contesting and before==0 else (row or {}).get('contested_at')
        ...
        polywar._safe_rollback(conn); logger.exception('Unexpected PolyWar combat failure'); raise
    finally: conn.close()
