...
def build_world_overview(user_id=None):
    ...
    try:
        ...
        if squads_enabled and 'polywar_faction_squads' in tables:
            active_squads = [dict(r) for r in polywar._fetchall(conn.cursor(), "SELECT id,faction_id,x,y,supply_x,supply_y,hp,max_hp,status,target_x,target_y,attack_target_x,attack_target_y,attack_progress,reinforcement_at,expires_at FROM polywar_faction_squads WHERE season_id=%s AND expires_at>CURRENT_TIMESTAMP AND status IN ('spawning','marching','engaged','attacking_cell','pressuring_capital','waiting_for_supply','waiting_for_players','retreating','awaiting_reinforcement') LIMIT 14", (sid,))]
        rifts = []
