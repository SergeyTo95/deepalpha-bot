...
def build_world_overview(user_id=None):
    ...
    try:
        ...
        latest_squad = _latest(conn, 'polywar_faction_squads', sid) if 'polywar_faction_squads' in _tables(conn) else None
        latest_pressure = _latest(conn, 'polywar_squad_pressure', sid) if 'polywar_squad_pressure' in _tables(conn) else None
        key = (sid, config.width, config.height, config.sector_size, cols, rows, latest_sector, latest_cap, latest_rift, latest_squad, latest_pressure)
