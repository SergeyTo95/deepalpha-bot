...
def _call_gemini_vision_parts(api_key: str, model: str, timeout: int, parts: List[Dict[str, Any]], max_tokens: int, *, user_id: Optional[int] = None, access_checked: bool = False) -> Tuple[str, str]:
    ...
    logger.info("gemini_call_started user_id=%s feature=live_analyst", user_id)
    payload = {
        "contents": [{"parts": parts}],
        "generationConfig": {"maxOutputTokens": max_tokens, "temperature": 0.1},
    }
    result = _post_gemini_generate_content(api_key, model, timeout, payload, max_tokens)
    logger.info("gemini_call_completed user_id=%s feature=live_analyst", user_id)
    return result
