...
html{-webkit-text-size-adjust:100%;scroll-padding-bottom:calc(82px + env(safe-area-inset-bottom))}
body{min-width:0;overflow-x:hidden;overscroll-behavior-y:contain}
...
def _mobile_table(table_match: re.Match[str]) -> str:
    ...
    if re.search(r"\bclass=(['\"])", attrs, re.IGNORECASE):
        attrs = re.sub(r"\bclass=(['\"])(.*?)\1", lambda m: f"class={m.group(1)}{m.group(2)} mobile-card-table{m.group(1)}", attrs, count=1, flags=re.IGNORECASE | re.DOTALL)
    else:
        attrs += " class='mobile-card-table'"
