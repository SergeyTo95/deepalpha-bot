...
def capture_cell(user_id: int, x: int, y: int, idempotency_key: str):
    ...
    try:
        ...
        if polywar._is_sqlite(conn):
            c.execute('SAVEPOINT squad_capture_integration')
        else:
            polywar._execute(c, 'SAVEPOINT squad_capture_integration')
        try:
            ...
            polywar._execute(c, 'RELEASE SAVEPOINT squad_capture_integration')
        except Exception:
            polywar._execute(c, 'ROLLBACK TO SAVEPOINT squad_capture_integration')
            polywar._execute(c, 'RELEASE SAVEPOINT squad_capture_integration')
