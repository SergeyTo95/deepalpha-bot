...
def _release_candidates(execution_module: Any, limit: int) -> list[tuple[int, str]]:
    ...
    try:
        cursor.execute(
            ...
            WHERE e.status='review_ready' AND COALESCE(s.status,'ready') <> 'complete'
            ORDER BY e.updated_at ASC LIMIT %s
            """,
...
def _progress_release(execution_module: Any, user_id: int, execution_id: str) -> Dict[str, Any]:
    ...
    if str(candidate.get("status") or "") != "eligible" or not bool(candidate.get("release_eligible")):
        return _save_state(
            ...
            candidate_id=str(candidate.get("candidate_id") or ""), status="blocked",
            blocker_code="velia_factory_stage8_candidate_not_eligible",
        )
    _assert_repository_scope(int(user_id), candidate)
    candidate_id = str(candidate.get("candidate_id") or "")
    state = _save_state(execution_module, int(user_id), str(execution_id), candidate_id=candidate_id, status="approved", blocker_code="", blocker_detail="")
    ...
    plan = (
        execution_module.get_release_preflight(int(user_id), state["plan_id"])
        if state.get("plan_id")
        else execution_module.prepare_release_preflight(int(user_id), candidate_id)
    )
    plan_id = str(plan.get("plan_id") or "")
    execution_module.validate_release_preflight(int(user_id), plan_id)
    state = _save_state(execution_module, int(user_id), str(execution_id), plan_id=plan_id, status="preflight_passed")

    release = (
        execution_module.get_release_execution(int(user_id), state["release_execution_id"])
        if state.get("release_execution_id")
        else execution_module.create_release_execution(int(user_id), plan_id)
    )
    release_id = str(release.get("execution_id") or "")
    state = _save_state(execution_module, int(user_id), str(execution_id), release_execution_id=release_id, status="merging")
    ...
    if release_status != "completed":
        return _save_state(
            execution_module, int(user_id), str(execution_id), status="release_" + (release_status or "unknown"),
            blocker_code=str(release.get("error_code") or ""), blocker_detail=str(release.get("error_detail") or ""),
        )
    ...
    if str(verification.get("verification_status") or "") != "verified":
        return _save_state(execution_module, int(user_id), str(execution_id), verification_id=verification_id, status="verification_failed", blocker_code="velia_factory_stage8_release_verification_failed")
    state = _save_state(execution_module, int(user_id), str(execution_id), verification_id=verification_id, status="deployment_observing")
    ...
    state = _save_state(execution_module, int(user_id), str(execution_id), observation_id=observation_id, status="deployment_" + (observation_status or "unknown"))
    if observation_status != "success" or not bool(observation.get("deployment_complete")):
        return state

    certificate = execution_module.evaluate_release_completion(int(user_id), verification_id, observation_id)
    certificate_id = str(certificate.get("certificate_id") or "")
    certificate_status = str(certificate.get("status") or "")
    state = _save_state(execution_module, int(user_id), str(execution_id), certificate_id=certificate_id, status="acceptance_" + (certificate_status or "unknown"))
    ...
    return _save_state(
        execution_module, int(user_id), str(execution_id), passport_id=passport_id,
        status="complete", blocker_code="", blocker_detail="",
    )
    ...
    def run_workspace_supervisor_once() -> list[Dict[str, Any]]:
        ...
        for user_id, execution_id in _release_candidates(execution_module, limit):
            ...
            try:
                ...
                try:
                    _save_state(execution_module, int(user_id), str(execution_id), status="blocked", blocker_code=code, blocker_detail=detail)
                except Exception:
                    logger.exception("VELIA_STAGE8_RELEASE_STATE_SAVE_FAILED execution_id=%s", execution_id)
                logger.exception("VELIA_STAGE8_RELEASE_COORDINATOR_FAILED execution_id=%s code=%s", execution_id, code)
        return results
