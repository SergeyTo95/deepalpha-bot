...
def install(chat_module: Any, workspace_chat_service: Any, runtime_module: Any) -> None:
    ...
    # It owns greenfield intake/attach and delegates to the existing Factory
    # paths after exact repository attachment. Stage 8 wraps Stage 4.5 only after
    # its hardening is installed, so automatic repository creation cannot bypass
    # the established owner/scope checks.
    try:
        ...
        if getattr(chat_module, "_velia_software_factory_greenfield_hardening_installed", False):
            stage8_greenfield_runtime.install(chat_module, greenfield_service, greenfield_runtime)
    except Exception:
