...
def _live_image_resolved_confidence(resolved_market: dict = None) -> float:
    if not resolved_market:
        return 0.0
    try:
        return float(resolved_market.get("confidence") or 0.0)
...
async def live_image_educational_callback(callback: types.CallbackQuery):
    ...
    if callback.data == LIVE_IMAGE_FULL_ANALYSIS_HELP_CALLBACK:
        text = (
            "🔍 Лучший вариант — отправь ссылку Polymarket. Тогда я смогу проверить правила, "
            "цены, liquidity/spread и сравнить market price с AI probability.\n\n"
            "После полной проверки я дам итог EDGE / NO TRADE через обычный анализ."
        )
    elif callback.data == LIVE_IMAGE_EXPLAIN_EDGE_CALLBACK:
        text = (
            "🧠 Edge — это разница между ценой рынка и независимой оценкой вероятности.\n\n"
            "Пример: если YES стоит около 51%, а аккуратная оценка даёт 60%, может быть потенциальное преимущество.\n\n"
            "Перед выводом проверь: resolution rules, liquidity/spread, свежесть новостей и не учтена ли информация уже в цене. "
            "Маленький edge легко исчезает из-за spread/slippage, поэтому финальный EDGE / NO TRADE нужен только после полного контекста."
        )
    else:
        text = (
            "⚠️ Главные риски Polymarket:\n"
            "• resolution ambiguity: формулировка правил может решить исход;\n"
            "• liquidity/spread: цена на экране может быть неисполняемой нормальным объёмом;\n"
            "• headline risk: поздняя новость быстро меняет вероятность;\n"
            "• screenshot limitations: скрин не показывает все правила и глубину рынка;\n"
            "• late entry / overconfidence: очевидная история часто уже заложена в цену."
        )
