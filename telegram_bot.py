...
async def history_handler(message: types.Message):
    ...
    for r in records:
        ...
        lines.append("")
    await message.answer("\n".join(lines), reply_markup=get_main_keyboard(uid), parse_mode="Markdown")
...
# ✅ Исправлено — ловит любую ссылку polymarket.com
...
def _format_analysis(result: dict, uid: int) -> str:
    return (
        ...
        f"*{t(uid, 'question')}:* {result.get('question', '')}\n"
        f"*{t(uid, 'category')}:* {result.get('category', '')}\n"
        f"*{t(uid, 'market_probability')}:* {result.get('market_probability', '')}\n"
        f"*{t(uid, 'system_probability')}:* {result.get('probability', '')}\n"
        f"*{t(uid, 'confidence')}:* {result.get('confidence', '')}\n\n"
