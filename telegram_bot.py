...
from services.jarvis_service import (
    build_help_text, can_spend, generate_jarvis_response, get_jarvis_settings,
...
from services.airdrop_share_card_service import (
...
LIVE_STATUS_BUTTON = "📊 Live Status"
LIVE_RESET_BUTTON = "🔄 Сбросить Live"
LIVE_EXIT_BUTTON = "⬅️ Выйти из Live"
LIVE_ADMIN_BUTTON = "🧠 Live Admin"
LIVE_STATS_BUTTON = "📊 Live Stats"

LIVE_ANALYST_LEGACY_RESET_BUTTON = "🔄 Reset Live context"
LIVE_ANALYST_LEGACY_EXIT_BUTTON = "⬅️ Exit Live Mode"
...
def format_airdrop_teaser(user_id: int, ui_language: str = "ru") -> str:
    base = format_airdrop_status(user_id, ui_language)
    hint = "\n\n🎯 Daily Quests: /quests"
    return f"{base}{hint}"
...
def _extract_share_summary(text: str) -> str:
    clean = re.sub(r"<[^>]+>", "", str(text or ""))
    clean = re.sub(r"\s+", " ", clean).strip()
    return clean[:177] + "…" if len(clean) > 180 else clean


def _save_analysis_share_card(user_id: int, analysis_type: str, title: str = "", market_url: str = "", domain: str = "", decision: str = "WATCH", score=None, short_summary: str = "", key_risk: str = "", metadata: dict = None, ui_language: str = "ru") -> dict:
    payload = build_share_card_payload(
        user_id=user_id, analysis_type=analysis_type, title=title, market_url=market_url, domain=domain,
        decision=decision, score=score, short_summary=short_summary, key_risk=key_risk, metadata=metadata or {}, ui_language=ui_language,
    )
    return save_latest_share_card(user_id, payload)


def get_airdrop_share_keyboard(user_id: int) -> InlineKeyboardMarkup:
    lang = get_user_lang(user_id)
    kb = InlineKeyboardMarkup(row_width=1)
    kb.add(InlineKeyboardButton("📤 Поделиться инсайтом" if lang == "ru" else "📤 Share insight", callback_data="airdrop_share_latest"))
    return kb


def format_share_earn_screen(user_id: int, ui_language: str = "ru") -> str:
    from services.airdrop_referral_service import get_or_create_referral_code
    code = get_or_create_referral_code(user_id)
    referral_link = f"https://t.me/{(BOT_USERNAME or 'DeepAlphaAI_bot').lstrip('@')}?start=ref_{code}"
    stats = get_share_card_stats(user_id)
    active_referrals = get_referral_activation_count(user_id)
    if ui_language == "en":
        return (f"📤 Share & Earn\n\nRun a DeepAlpha analysis and share the insight with your referral link.\n\nHow it works:\n1. Run market/event analysis\n2. Tap “📤 Share insight”\n3. Share the card in Telegram/X/chat\n4. People join through your link\n5. Referral rewards unlock when they become active\n\nYour referral link:\n{referral_link}\n\nToday:\n• Share-cards: {stats.get('share_cards_generated_today', 0)}\n• Total share-cards: {stats.get('total_share_cards_generated', 0)}\n• Active referrals: {active_referrals}")
    return (f"📤 Share & Earn\n\nСделай анализ в DeepAlpha и поделись инсайтом со своей referral-ссылкой.\n\nКак это работает:\n1. Запусти анализ рынка или события\n2. Нажми “📤 Поделиться инсайтом”\n3. Отправь карточку в Telegram/X/чат\n4. Люди заходят по твоей ссылке\n5. Реферальные награды начисляются, когда они становятся активными\n\nТвоя referral-ссылка:\n{referral_link}\n\nСегодня:\n• Share-cards: {stats.get('share_cards_generated_today', 0)}\n• Всего share-cards: {stats.get('total_share_cards_generated', 0)}\n• Активных рефералов: {active_referrals}")
...
def get_analysis_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    ...
    if lang == "ru":
        ...
        kb.add(KeyboardButton(LIVE_ANALYST_BUTTON), KeyboardButton("🏁 Market Recap"))
...
def get_live_analyst_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    ...
    kb.add(KeyboardButton(LIVE_STATUS_BUTTON))
    kb.add(KeyboardButton(LIVE_RESET_BUTTON))
    kb.add(KeyboardButton(LIVE_EXIT_BUTTON))
    return kb
...
def get_help_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    ...
    kb = ReplyKeyboardMarkup(resize_keyboard=True)
    kb.add(KeyboardButton(LIVE_ANALYST_BUTTON))
    kb.add(KeyboardButton("⬅️ Назад" if lang == "ru" else "⬅️ Back"))
    return kb
...
def get_live_admin_keyboard() -> InlineKeyboardMarkup:
    settings = get_settings_snapshot()
    kb = InlineKeyboardMarkup(row_width=2)
    if settings.get("enabled"):
        kb.add(InlineKeyboardButton("⛔ Disable Live", callback_data="live_admin_toggle:off"))
    else:
        kb.add(InlineKeyboardButton("✅ Enable Live", callback_data="live_admin_toggle:on"))
    if settings.get("images_enabled"):
        kb.add(InlineKeyboardButton("🖼 Disable Images", callback_data="live_admin_images:off"))
    else:
        kb.add(InlineKeyboardButton("🖼 Enable Images", callback_data="live_admin_images:on"))
    kb.add(InlineKeyboardButton(LIVE_STATS_BUTTON, callback_data="live_admin_stats"))
    kb.add(
        InlineKeyboardButton("💬 Text price", callback_data="live_admin_set_text_price"),
        InlineKeyboardButton("🖼 Image price", callback_data="live_admin_set_image_price"),
    )
...
LIVE_IMAGE_EVENT_BUNDLE_MARKETS_CALLBACK = "live_img_event_bundle_markets"
...
def _live_image_resolved_confidence(resolved_market: dict = None) -> float:
    if not resolved_market:
        return 0.0
    confidence = resolved_market.get("confidence")
...
def _is_live_image_strong_market_match(resolved_market: dict = None) -> bool:
    return _live_image_resolved_confidence(resolved_market) >= LIVE_IMAGE_STRONG_CONFIDENCE_THRESHOLD


def _is_live_image_medium_market_match(resolved_market: dict = None) -> bool:
    confidence = _live_image_resolved_confidence(resolved_market)
    return LIVE_IMAGE_MEDIUM_CONFIDENCE_THRESHOLD <= confidence < LIVE_IMAGE_STRONG_CONFIDENCE_THRESHOLD
...
def _remember_live_image_event_bundle(user_id: int, bundle: dict = None) -> None:
...
def _remember_live_image_candidate(user_id: int, resolved_market: dict = None) -> None:
    if not resolved_market or not resolved_market.get("url"):
        ...
        return
    LIVE_IMAGE_CANDIDATE_MARKETS[user_id] = {
...
def _clear_live_image_market_resolution(user_id: int) -> None:
    LIVE_IMAGE_STRONG_MARKET_CONFIDENCE.pop(user_id, None)
    LIVE_IMAGE_CANDIDATE_MARKETS.pop(user_id, None)
    LIVE_IMAGE_EVENT_BUNDLE_CONTEXT.pop(user_id, None)
...
def _format_event_bundle_lines(bundle: dict, lang: str = "ru") -> str:
    lines = []
    for market in (bundle or {}).get("markets", [])[:8]:
        original = str(market.get("entity_original") or market.get("entity_canonical") or "").strip()
        canonical = str(market.get("entity_canonical") or original).strip()
        prob = market.get("visible_probability")
        try:
            prob_f = float(prob)
            prob_s = str(int(prob_f)) if prob_f.is_integer() else str(prob_f).rstrip("0").rstrip(".")
            prefix = f"{original} — {prob_s}%"
        except (TypeError, ValueError):
            prefix = original
        if lang != "en":
            lines.append(f"• {prefix} → найден рынок {canonical}")
        else:
            lines.append(f"• {prefix} → found {canonical} market")
    return "\n".join(lines)


def format_live_image_resolution_notice(resolved_market: dict = None, lang: str = "ru") -> str:
    is_ru = lang != "en"
    if resolved_market and resolved_market.get("type") == "event_bundle":
        title = str(resolved_market.get("event_title") or resolved_market.get("title") or "Event").strip()[:300]
        lines = _format_event_bundle_lines(resolved_market, "ru" if is_ru else "en")
        if resolved_market.get("confidence") == "medium":
            if is_ru:
...
def get_live_image_keyboard(resolved_market: dict = None, lang: str = "ru") -> InlineKeyboardMarkup:
    ...
    if resolved_market and resolved_market.get("type") == "event_bundle":
        if resolved_market.get("confidence") == "strong":
            kb.add(InlineKeyboardButton("⚡ Быстрый анализ события" if is_ru else "⚡ Quick event analysis", callback_data=LIVE_IMAGE_RUN_FULL_ANALYSIS_CALLBACK))
...
def _event_bundle_markets_text(bundle: dict, lang: str = "ru") -> str:
    is_ru = lang != "en"
...
def _build_event_bundle_quick_analysis_text(bundle: dict, lang: str = "ru") -> str:
    is_ru = lang != "en"
    title = str(bundle.get("event_title") or bundle.get("title") or "Event")
    rows = []
    for m in (bundle.get("markets") or []):
        entity = str(m.get("entity_canonical") or m.get("entity") or "")
        visible = m.get("visible_probability")
        current = m.get("current_probability")
        def fmt(v):
            try:
                return f"{float(v):g}%"
            except (TypeError, ValueError):
                return "n/a"
        diff_s = "n/a"
        note = "no clear discrepancy" if not is_ru else "нет явного расхождения"
        try:
...
async def _run_top_analysis_for_user(uid: int, lang: str, analysis: dict, respond_fn) -> None:
    ...
    try:
        ...
        if new_balance == 0 and user_balance > 0:
            ...
            await respond_fn(_get_top_analysis_balance_message(lang, price), reply_markup=get_pay_keyboard(lang))
            return
        top_text = _format_top_analysis_output(lang, input_data.get("question", ""), result)
        _save_analysis_share_card(uid, "top_analysis", title=input_data.get("question", ""), domain="top_analysis", decision="WATCH", short_summary=_extract_share_summary(top_text), key_risk="Resolution rules, liquidity, and missing data can materially change the interpretation.", metadata={"source": "top_analysis"}, ui_language=lang)
        await respond_fn(top_text, reply_markup=get_airdrop_share_keyboard(uid))
        try:
...
def _jarvis_team_allowed(message: types.Message, command: str) -> bool:
    if not is_team_chat(message.chat.id):
        return False
    if command in JARVIS_TEAM_COMMANDS:
        return True
    if message.reply_to_message and command in JARVIS_TEAM_COMMANDS:
        return True
    return False
...
async def _jarvis_alert_founders(message: types.Message, reason: str) -> None:
    settings = get_jarvis_settings()
    if not settings.alerts_enabled:
        return
    safe_reason = (reason or "Jarvis error")[:80]
    for founder_id in settings.founder_ids:
        try:
            await bot.send_message(founder_id, f"🧠 Jarvis alert: {safe_reason}")
        except Exception:
            logger.info(
                "jarvis_alert_failed user_id=%s chat_id=%s",
                founder_id,
...
async def _run_jarvis_llm_command(message: types.Message, command: str, args: str, scope: str) -> None:
    ...
    try:
        ...
        logger.info("jarvis_command command=%s user_id=%s chat_id=%s success=true", command, actor_id, message.chat.id)
    except Exception as exc:
        logger.info(
            "jarvis_command command=%s user_id=%s chat_id=%s success=false error=%s",
            command,
            actor_id,
            message.chat.id,
...
@dp.message_handler(commands=["jarvis", "jarvis_help", "post", "reply", "today", "jarvis_status", "ask", "tokens", "stats"], state="*")
async def jarvis_command_handler(message: types.Message, state: FSMContext):
    await state.finish()
    command = _jarvis_command_name(message)
    args = message.get_args() or ""
    founder = bool(message.from_user and is_founder_user(message.from_user.id))
    ...
    if command in {"jarvis", "ask"} and not founder:
        return
    if not founder and not _jarvis_team_allowed(message, command):
        return

    if command in {"jarvis", "jarvis_help"}:
        await message.answer(build_help_text(founder=founder, team=team))
        return

    if command == "jarvis_status":
        await message.answer(_format_jarvis_status(message.from_user.id, founder=True))
        return

    if command == "tokens":
        await message.answer(_format_jarvis_tokens(message.from_user.id, founder=True))
        return

    if command == "stats":
        if founder or team:
            await message.answer(_format_jarvis_stats())
        return
...
@dp.message_handler(lambda m: is_team_chat(m.chat.id) and m.text and not (m.text or "").startswith("/") and re.search(r"\b(jarvis|джарвис)\b", m.text, re.IGNORECASE), state="*")
async def jarvis_team_mention_handler(message: types.Message, state: FSMContext):
    await state.finish()
...
async def analyst_profile_command(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        logger.warning("airdrop_profile_quest_failed user_id=%s error=%s", uid, exc)
...
async def analyst_profile_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        record_profile_daily_quest(uid, source="analyst_profile_callback", metadata={"callback": data})
    except Exception as exc:
        logger.warning("airdrop_profile_quest_callback_failed user_id=%s error=%s", uid, exc)
...
async def user_help_handler(message: types.Message, state: FSMContext):
    ...
    await message.answer(text, reply_markup=private_reply_markup(message, get_help_keyboard(uid)))
...
def _live_status_text(uid: int) -> str:
    ...
    return (
        ...
        f"Text cost: {billing['text_cost']} token{'s' if billing['text_cost'] != 1 else ''}\n"
        f"Image cost: {billing['image_cost']} token{'s' if billing['image_cost'] != 1 else ''}\n"
        f"Balance: {billing['balance']} tokens"
    )


def _live_admin_stats_text() -> str:
...
async def _activate_live_mode(message: types.Message, analysis: Optional[dict] = None, already_text: bool = False) -> None:
    _register_user(message)
    uid = message.from_user.id
    if _check_banned(message):
        await message.answer(t(uid, "banned"))
    ...
    if not is_live_enabled():
        ...
        return
    if already_text and is_live_session_active(uid) and not analysis:
        await message.answer(
            "🧠 Live Analyst уже включён. Можешь отправить вопрос, скрин или уточнение.",
...
async def _exit_live_mode(message: types.Message) -> None:
    _register_user(message)
    uid = message.from_user.id
    exit_live_session(uid)
    await message.answer(
        "Live Analyst выключен. Обычный режим бота восстановлен.",
        reply_markup=private_reply_markup(message, get_main_keyboard(uid)),
    )


async def _reset_live_mode(message: types.Message) -> None:
    _register_user(message)
    uid = message.from_user.id
...
async def _show_live_status(message: types.Message) -> None:
    _register_user(message)
    uid = message.from_user.id
    await message.answer(
        _live_status_text(uid),
        reply_markup=private_reply_markup(message, get_live_analyst_keyboard(uid) if is_live_session_active(uid) else get_main_keyboard(uid)),
    )
...
async def live_command_handler(message: types.Message, state: FSMContext):
    if not _is_private_live_message(message):
        return
    await state.finish()
    await _activate_live_mode(message, already_text=True)


@dp.message_handler(lambda m: (m.text or "") == LIVE_ANALYST_BUTTON, state="*")
...
@dp.message_handler(lambda m: (m.text or "") in {LIVE_EXIT_BUTTON, LIVE_ANALYST_LEGACY_EXIT_BUTTON}, state="*")
async def exit_live_handler(message: types.Message, state: FSMContext):
    if not _is_private_live_message(message):
        return
    await state.finish()
    await _exit_live_mode(message)


@dp.message_handler(commands=["reset_live"], state="*")
@dp.message_handler(lambda m: (m.text or "") in {LIVE_RESET_BUTTON, LIVE_ANALYST_LEGACY_RESET_BUTTON}, state="*")
async def reset_live_handler(message: types.Message, state: FSMContext):
    if not _is_private_live_message(message):
        return
    await state.finish()
    await _reset_live_mode(message)


@dp.message_handler(commands=["live_status"], state="*")
@dp.message_handler(lambda m: (m.text or "") == LIVE_STATUS_BUTTON, state="*")
async def live_status_handler(message: types.Message, state: FSMContext):
    if not _is_private_live_message(message):
        return
    await state.finish()
    await _show_live_status(message)
...
async def live_discuss_callback(callback: types.CallbackQuery, state: FSMContext):
    if not _is_private_live_callback(callback):
        await callback.answer("Live Analyst доступен только в личном чате с ботом.", show_alert=True)
        return
    await state.finish()
    uid = callback.from_user.id
...
async def live_access_admin_handler(message: types.Message, state: FSMContext):
    ...
    if cmd == "airdrop_share_status":
        ...
        domains = "\n".join([f"• {k}: {v}" for k, v in (st.get("domains") or {}).items()]) or "—"
        await message.answer(f"Airdrop share-card status\n\nTotal share cards: {st.get('total_share_cards_generated', 0)}\nToday: {st.get('share_cards_generated_today', 0)}\nUsers: {st.get('users_who_generated_cards', 0)}\n\nTop users:\n{top}\n\nDomains:\n{domains}")
        return
    ...
    if cmd == "airdrop_quests_status":
        ...
        top_lines = [f"- {row.get('quest_code')}: {row.get('count')}" for row in top]
        await message.answer(
            "Airdrop Quests Status:\n\n"
            f"- Date: {status.get('quest_date')}\n"
            f"- Points enabled: {str(status.get('points_enabled')).lower()}\n"
            f"- Total completions today: {status.get('total_completions')}\n"
            f"- Total quest points awarded today: {status.get('total_points_awarded')}\n\n"
            "Top quest codes today:\n" + ("\n".join(top_lines) if top_lines else "—")
        )
        return
...
async def live_admin_handler(message: types.Message, state: FSMContext):
    ...
    if (message.get_command() or "").lstrip("/") == "live_stats":
        await message.answer(_live_admin_stats_text())
        return
    await _send_live_admin_panel(message)


@dp.message_handler(lambda m: (m.text or "") == LIVE_ADMIN_BUTTON, state="*")
async def live_admin_button_handler(message: types.Message, state: FSMContext):
    if not message.from_user or not _require_live_admin(message.from_user.id):
        await message.answer("Команда доступна только администратору.")
        return
    await state.finish()
...
@dp.callback_query_handler(lambda c: c.data and (c.data == "live_admin_panel" or c.data.startswith("live_admin_")), state="*")
async def live_admin_callback(callback: types.CallbackQuery, state: FSMContext):
    if not callback.from_user or not _require_live_admin(callback.from_user.id):
        await callback.answer("Команда доступна только администратору.", show_alert=True)
        return
    data = callback.data or ""
    await state.finish()
    text = format_live_admin_text()
    markup = get_live_admin_keyboard()
    if data == "live_admin_toggle:on":
        set_live_setting("live_enabled", "true")
        text = format_live_admin_text()
        markup = get_live_admin_keyboard()
        await callback.answer("Live Analyst включён")
    elif data == "live_admin_toggle:off":
        set_live_setting("live_enabled", "false")
        text = format_live_admin_text()
        markup = get_live_admin_keyboard()
        await callback.answer("Live Analyst выключен")
    elif data == "live_admin_images:on":
        set_live_setting("image_analysis_enabled", "true")
        text = format_live_admin_text()
        markup = get_live_admin_keyboard()
        await callback.answer("Images enabled")
    elif data == "live_admin_images:off":
        set_live_setting("image_analysis_enabled", "false")
        text = format_live_admin_text()
        markup = get_live_admin_keyboard()
        await callback.answer("Images disabled")
    elif data == "live_admin_stats":
        text = _live_admin_stats_text()
        await callback.answer()
    elif data == "live_admin_set_text_price":
        text = "Отправь команду: /live_price text <tokens>"
        await callback.answer()
    elif data == "live_admin_set_image_price":
        text = "Отправь команду: /live_price image <tokens>"
        await callback.answer()
    elif data == "live_admin_set_memory_limit":
        text = "Отправь команду: /live_memory <count>"
        await callback.answer()
    else:
        await callback.answer()
    try:
...
async def live_admin_toggle_handler(message: types.Message, state: FSMContext):
    ...
    if cmd == "live_on":
        set_live_setting("live_enabled", "true")
        await message.answer("Live Analyst включён.", reply_markup=get_live_admin_keyboard())
    elif cmd == "live_off":
        set_live_setting("live_enabled", "false")
        await message.answer("Live Analyst выключен.", reply_markup=get_live_admin_keyboard())
    elif cmd == "live_image_on":
        set_live_setting("image_analysis_enabled", "true")
        await message.answer("Image analysis включён.", reply_markup=get_live_admin_keyboard())
    else:
        set_live_setting("image_analysis_enabled", "false")
        await message.answer("Image analysis выключен.", reply_markup=get_live_admin_keyboard())
...
async def live_price_handler(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"Live price updated: {parts[1]} = {price} tokens", reply_markup=get_live_admin_keyboard())
...
async def live_memory_handler(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"Live memory updated: last {count} messages", reply_markup=get_live_admin_keyboard())
...
async def live_image_handler(message: types.Message, state: FSMContext):
    ...
    if result.get("screen_type") == "polymarket":
        ...
        if result.get("market"):
            try:
                ...
                resolved_market = None
        if resolved_market and resolved_market.get("type") == "event_bundle" and _is_live_image_strong_market_match(resolved_market):
            LIVE_IMAGE_STRONG_MARKET_CONFIDENCE[uid] = _live_image_resolved_confidence(resolved_market)
            _remember_live_image_event_bundle(uid, resolved_market)
            LIVE_IMAGE_SCREENSHOT_NO_MATCH_CONTEXT.pop(uid, None)
        elif resolved_market and resolved_market.get("type") == "event_bundle" and _is_live_image_medium_market_match(resolved_market):
    ...
    if result.get("screen_type") == "polymarket":
        ...
        if notice_market and notice_market.get("type") == "event_bundle":
            logger.info("live_screenshot_event_bundle_message_sent user_id=%s matched=%s event_url=%s", uid, notice_market.get("matched_entities_count"), notice_market.get("event_url") or notice_market.get("market_url"))
    else:
        LIVE_IMAGE_SCREENSHOT_NO_MATCH_CONTEXT.pop(uid, None)
...
@dp.callback_query_handler(lambda c: c.data in {
    ...
    LIVE_IMAGE_EVENT_BUNDLE_MARKETS_CALLBACK,
...
async def live_image_educational_callback(callback: types.CallbackQuery):
    ...
    if callback.data == LIVE_ANALYST_SCREENSHOT_ONLY_ANALYSIS_CALLBACK:
        ...
        try:
            ...
            await callback.answer()
    elif callback.data == LIVE_IMAGE_EVENT_BUNDLE_MARKETS_CALLBACK:
        uid = callback.from_user.id if callback.from_user else 0
        bundle = LIVE_IMAGE_EVENT_BUNDLE_CONTEXT.get(uid) or {}
        if bundle.get("type") == "event_bundle":
            lang = get_user_lang(uid)
            await callback.answer()
            await callback.message.answer(_event_bundle_markets_text(bundle, lang), reply_markup=_event_bundle_markets_keyboard(bundle, lang))
            return
        text = "Не вижу найденных рынков. Отправь ссылку Polymarket вручную."
...
async def live_image_retry_market_resolution_callback(callback: types.CallbackQuery):
    if not _is_private_callback(callback):
        await callback.answer(LIVE_IMAGE_PRIVATE_CHAT_ALERT, show_alert=True)
        return
    ...
    await callback.answer("Ищу рынок ещё раз…")
    try:
        resolved_market = await resolve_polymarket_market_from_screenshot(title, visible=visible)
    except Exception as exc:
        logger.warning("live_image_market_retry_resolve_failed user_id=%s reason=%s", uid, type(exc).__name__)
        resolved_market = None
    ...
    if resolved_market and resolved_market.get("type") == "event_bundle" and _is_live_image_medium_market_match(resolved_market):
        LIVE_IMAGE_STRONG_MARKET_CONFIDENCE.pop(uid, None)
        _remember_live_image_event_bundle(uid, resolved_market)
        await callback.message.answer(format_live_image_resolution_notice(resolved_market, get_user_lang(uid)), reply_markup=get_live_image_keyboard(resolved_market, get_user_lang(uid)))
        return
...
@dp.callback_query_handler(lambda c: c.data == LIVE_IMAGE_CONFIRM_CANDIDATE_ANALYSIS_CALLBACK)
async def live_image_confirm_candidate_analysis_callback(callback: types.CallbackQuery):
    if not _is_private_callback(callback):
        await callback.answer(LIVE_IMAGE_PRIVATE_CHAT_ALERT, show_alert=True)
        return
    if not callback.from_user or not callback.message:
    ...
    candidate_confidence = _live_image_resolved_confidence(candidate)
    if not candidate_url:
        await callback.answer()
        await callback.message.answer("Не вижу кандидата для анализа. Нажми поиск ещё раз или отправь ссылку Polymarket.")
        return
    if candidate_confidence < LIVE_IMAGE_MEDIUM_CONFIDENCE_THRESHOLD:
        await callback.answer()
        await callback.message.answer("Совпадение слишком слабое. Нажми поиск ещё раз или отправь ссылку Polymarket.")
        return
    if not _is_polymarket_url(candidate_url):
        await callback.answer()
        await callback.message.answer("Кандидат не похож на ссылку Polymarket. Отправь ссылку вручную.")
        return
...
async def live_image_run_full_analysis_callback(callback: types.CallbackQuery):
    ...
    if stored_confidence is None or stored_confidence < LIVE_IMAGE_STRONG_CONFIDENCE_THRESHOLD:
        await callback.answer()
        await callback.message.answer(
            "Я не уверен, что найден правильный рынок. Нажми 🔍 Анализ и отправь ссылку Polymarket вручную."
        )
        return
...
async def _run_normal_polymarket_analysis(message: types.Message, url_override: str = "", user_id_override: int = 0):
    ...
    try:
        ...
        _save_analysis_share_card(uid, "telegram_quick_analysis", title=result.get("question") or result.get("market") or "DeepAlpha market insight", market_url=url, domain=result.get("category_type") or result.get("category") or result.get("analysis_mode") or "polymarket", decision=result.get("decision") or result.get("alpha_label") or "WATCH", score=result.get("deepalpha_score") or result.get("score"), short_summary=_extract_share_summary(text), key_risk=result.get("risk") or result.get("risks") or "Resolution rules, liquidity, and missing data can materially change the interpretation.", metadata={"source": "telegram_quick_analysis"}, ui_language=lang)
...
async def ton_send_flow(message: types.Message):
    ...
    if st["step"] == "amount":
        try:
            ...
            balance_data = get_user_ton_balance(message.from_user.id, refresh=True)
            if not balance_data.get("ok"):
                await message.answer(_ton_send_error(message.from_user.id, "balance_unavailable"))
                return
            balance_nano = int(balance_data.get("balance_nano") or 0)
            reserve_nano = get_ton_send_fee_reserve_nano()
            if nano + reserve_nano > balance_nano:
                balance_disp = balance_data.get("balance_display") or nano_to_ton_display(balance_nano)
        ...
        reserve_nano = get_ton_send_fee_reserve_nano()
        reserve_disp = nano_to_ton_display(reserve_nano)
        total_disp = nano_to_ton_display(int(st["amount_nano"]) + reserve_nano)
        kb = InlineKeyboardMarkup(row_width=2)
