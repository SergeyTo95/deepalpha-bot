...
async def live_image_educational_callback(callback: types.CallbackQuery):
    ...
    if callback.data == LIVE_IMAGE_FULL_ANALYSIS_HELP_CALLBACK:
        text = (
            "🔍 Лучший вариант — отправь ссылку на рынок Polymarket.\n\n"
            "Тогда я смогу проверить:\n"
            "• правила resolution;\n"
            "• текущие цены;\n"
            "• liquidity/spread;\n"
            "• свежие новости;\n"
            "• разницу между market price и AI-вероятностью.\n\n"
            "После этого можно дать нормальный вывод: EDGE или NO TRADE."
        )
    elif callback.data == LIVE_IMAGE_EXPLAIN_EDGE_CALLBACK:
        text = (
            "🧠 Edge — это разница между ценой рынка и твоей оценкой вероятности.\n\n"
            "Пример:\n"
            "если рынок даёт исходу 51%, а аккуратный анализ даёт 60%, появляется потенциальное преимущество.\n\n"
            "Но перед выводом нужно проверить:\n"
            "• правила resolution;\n"
            "• свежие новости;\n"
            "• liquidity и spread;\n"
            "• не заложена ли информация уже в цену.\n\n"
            "Маленький edge легко исчезает из-за spread/slippage, поэтому финальный вывод EDGE / NO TRADE нужен только после полного анализа."
        )
    else:
        text = (
            "⚠️ Главные риски Polymarket:\n\n"
            "• неправильно понять правила resolution — формулировка может решить исход;\n"
            "• войти слишком поздно — очевидная история часто уже заложена в цену;\n"
            "• не учесть liquidity/spread — цена на экране может быть неисполняемой нормальным объёмом;\n"
            "• переоценить скрин — он не показывает все правила, глубину рынка и контекст;\n"
            "• пропустить свежую новость — вероятность может быстро измениться.\n\n"
            "Поэтому скрин помогает понять рынок, но для EDGE / NO TRADE нужна ссылка и полный анализ."
        )
