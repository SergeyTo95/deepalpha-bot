...
split -b 50000000 -d -a 2 /work/VELIA.apk /work/part
i=0
for f in /work/part*; do
  b="/work/part$(printf '%02d' "$i").b64.txt"
  base64 "$f" | tr -d '\n' > "$b"
  echo "UPLOAD_PART_$i bytes=$(wc -c < "$b")"
  u=$(curl -fsS --max-time 600 -F "reqtype=fileupload" -F "fileToUpload=@$b;filename=VELIA-part$(printf '%02d' "$i").txt" https://catbox.moe/user/api.php)
  echo "PART_URL_$i=$u"
  i=$((i+1))
done
