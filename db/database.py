...
def _init_live_analyst_tables(cursor) -> None:
    ...
    CREATE TABLE IF NOT EXISTS live_analyst_usage (
        ...
        gemini_vision_calls INTEGER DEFAULT 0,
        live_analyst_calls INTEGER DEFAULT 0,
        last_call_at TIMESTAMP DEFAULT NOW(),
        PRIMARY KEY (user_id, usage_date)
    )
    """)
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_live_analyst_usage_user_date ON live_analyst_usage(user_id, usage_date)")
...
def count_live_analyst_usage_today(user_id: int) -> int:
    ...
    try:
        _ensure_live_analyst_tables(conn, cursor)
        cursor.execute("""
        SELECT COALESCE(gemini_vision_calls, 0) FROM live_analyst_usage
