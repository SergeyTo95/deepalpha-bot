...
def create_analysis_check(
    ...
    try:
        ...
        INSERT INTO analysis_checks (
            ...
            unit_price_tokens, total_price_tokens, refunded_tokens, disabled_at
        )
        VALUES (%s, %s, %s, %s, %s, 0, %s, %s, %s, 'active', %s, %s, %s, 0, NULL)
        RETURNING *
        """, (
            code, created_by_user_id, bool(created_by_admin), check_type, int(max_activations), expires_at,
            bool(require_channel_sub), required_channel or "", now,
            int(unit_price_tokens or 0), int(total_price_tokens or 0),
        ))
        row = cursor.fetchone()
        conn.commit()
        return dict(row) if row else None
