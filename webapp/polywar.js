...
class PolyWarMap {
  ...
  async bootstrapInitialLoad() { this.initialLoadStarted = true; await this.ensureChunks(); if (this.destroyed) return; this.initialChunksReady = true; await Promise.allSettled([this.ensureSectors(), this.refreshCapitals(), this.refreshGovernance()]); await this.refreshSquads(); this.loadOverview(); }
  ...
  isChunkCooldownActive() { return Date.now() < this.chunkCooldownUntil; }
  ...
  flushScheduledChunkLoad() {
    if (this.destroyed) return Promise.resolve({ok:false,destroyed:true});
    const priorityKey=this.queuedPriorityChunkKey, includeVisible=this.queuedVisibleChunkLoad;
    this.queuedPriorityChunkKey=null; this.queuedVisibleChunkLoad=false;
    const keys=[];
    if (priorityKey) keys.push(priorityKey.split(",").map(Number));
    if (includeVisible) for (const c of this.visibleChunks()) { const k=c.join(","); if (k!==priorityKey && !this.failedChunks.has(k)) keys.push(c); }
    return this.ensureChunks({keys,includeVisible:false,forceRefresh:false,generation:this.loadSeq});
  }
  scheduleChunkCooldownRecovery() {
    if (this.destroyed) return;
    if (this.chunkRecoveryTimer) clearTimeout(this.chunkRecoveryTimer);
    this.chunkRecoveryTimer=setTimeout(() => { this.chunkRecoveryTimer=null; if (this.isChunkCooldownActive()) { this.scheduleChunkCooldownRecovery(); return; } this.chunkCooldownUntil=0; this.runChunkRecovery(); }, Math.max(1,this.chunkCooldownUntil-Date.now()));
  }
  runChunkRecovery() {
    if (this.destroyed) return Promise.resolve({ok:false,destroyed:true});
    if (this.chunkRecoveryPromise) return this.chunkRecoveryPromise;
    this.chunkRecoveryPromise=this.ensureChunks(null,{recovery:true,includeVisible:true,generation:this.loadSeq});
    return this.chunkRecoveryPromise.finally(() => { this.chunkRecoveryPromise=null; this.chunkLoadQueuedDuringCooldown=false; if(this.destroyed)return; this.updateChunkStatus({generation:this.loadSeq}); this.updatePanel(); this.requestDraw(); this.drawMinimap(); });
  }
  enterChunkCooldown(retryAfterSeconds) {
    ...
    this.scheduleChunkCooldownRecovery();
    this.updateChunkStatus(); this.updatePanel(); this.requestDraw();
  }
  async ensureChunks(forceKey = null, options = {}) {
    ...
    if (this.isChunkCooldownActive() && !options.recovery) return { ok: false, error: "rate_limited", cooldown: true };
    ...
    const allowFailedRetry = options.manualRetry === true || options.retryFailed === true;
    const visibleKeys = includeVisible ? visible.filter(([x,y]) => allowFailedRetry || !this.failedChunks.has(`${x},${y}`)).map(([x,y]) => [x,y,false]) : [];
    const wanted = [...new Map(explicitKeys.concat(visibleKeys).map(([x,y,forced]) => [`${x},${y}`, [x,y,forced]])).values()];
    ...
    const successfulKeys=new Set(), rateLimitedKeys=new Set(), terminalFailedKeys=new Set();
      ...
      promiseForThisBatch = (async () => {
        ...
        try {
          ...
          if (data?.ok) {
            ...
            data.chunks.forEach(ch => { const key = `${ch.chunk_x},${ch.chunk_y}`; returned.add(key); successfulKeys.add(key); this.cache.set(key, ch); });
            batchKeys.forEach(k => { if (!returned.has(k)) terminalFailedKeys.add(k); });
            this.pruneCache();
          } else if (data?.error === "rate_limited" || data?.httpStatus === 429) {
            batchKeys.forEach(k=>rateLimitedKeys.add(k));
            this.enterChunkCooldown(data?.retry_after_seconds);
          } else {
            batchKeys.forEach(k => terminalFailedKeys.add(k));
          }
    ...
    successfulKeys.forEach(key=>this.failedChunks.delete(key));
    terminalFailedKeys.forEach(k=>{if(!this.cache.has(k))this.failedChunks.add(k);});
  ...
  updateChunkStatus(options = {}) {
    if(options.generation!=null&&options.generation!==this.loadSeq)return;
    const visibleFailed=this.visibleFailedChunkKeys(), visibleLoading=this.visibleChunks().some(([x,y])=>this.loading.has(`${x},${y}`));
    if(visibleLoading){this.status("Loading chunks…");this.removeRetryMap();return;}
    if(this.isChunkCooldownActive()){this.status("Map requests cooling down…");this.showRetryMap({disabled:true});return;}
    if(visibleFailed.length){this.status("Map data unavailable");this.showRetryMap({disabled:false});return;}
    this.status("");this.removeRetryMap();
  }

  removeRetryMap() { document.getElementById("retryMapBtn")?.remove(); }

  showRetryMap(options = {}) {
    let btn = document.getElementById("retryMapBtn");
    const status = document.getElementById("chunkStatus");
    if(!btn){btn=document.createElement("button");btn.className="btn mini";btn.id="retryMapBtn";btn.onclick=()=>this.retryMapLoading();}
    btn.disabled=!!options.disabled; btn.textContent=options.disabled?"Retrying shortly…":"Retry map";
    status?.after(btn);
  }
  retryMapLoading() {
    if(this.destroyed)return Promise.resolve({ok:false,destroyed:true});
    if(this.isChunkCooldownActive()){this.updateChunkStatus();return Promise.resolve({ok:false,error:"rate_limited",cooldown:true});}
    if(this.chunkManualRetryPromise)return this.chunkManualRetryPromise;
    const keys=this.visibleFailedChunkKeys(); if(!keys.length){this.updateChunkStatus();return Promise.resolve({ok:true,skipped:true});}
    keys.forEach(k=>this.failedChunks.delete(k));
    this.chunkManualRetryPromise=this.ensureChunks({keys:keys.map(k=>k.split(",").map(Number)),includeVisible:false,forceRefresh:true,manualRetry:true,retryFailed:true,generation:this.loadSeq});
    return this.chunkManualRetryPromise.finally(()=>{this.chunkManualRetryPromise=null;if(this.destroyed)return;this.updateChunkStatus({generation:this.loadSeq});this.updatePanel();this.requestDraw();this.drawMinimap();});
  }
  ...
  select(x, y) { if (x < 0 || y < 0 || x >= this.state.map.width || y >= this.state.map.height) return; const changed=this.selected?.x !== x || this.selected?.y !== y; if (this.selected?.x !== x || this.selected?.y !== y) this.moreOpen = false; if(changed) this.selectionAnimationUntil = polywarReducedMotion() || this.visualLowPower ? 0 : performance.now() + POLYWAR_VISUALS.selectionAnimationMs; this.selected = { x, y }; const key=this.chunkKeyForCell(x,y); if(this.initialLoadStarted&&!this.cache.has(key)&&!this.loading.has(key)&&!this.chunkRequestsByKey.has(key)&&!this.failedChunks.has(key))this.scheduleChunkLoad({delay:80,priorityKey:key,includeVisible:true}); this.updatePanel(); this.requestDraw(); }
  ...
  updatePanel() {
    ...
    const selectedChunkKey=s.x==null?null:this.chunkKeyForCell(s.x,s.y), selectedReady=selectedChunkKey&&this.cache.has(selectedChunkKey);
    const selectedLoading=selectedChunkKey&&!selectedReady&&(this.loading.has(selectedChunkKey)||this.chunkRequestsByKey.has(selectedChunkKey));
    const selectedFailed=selectedChunkKey&&!selectedReady&&this.failedChunks.has(selectedChunkKey);
    const selectedCoolingDown=selectedChunkKey&&!selectedReady&&this.isChunkCooldownActive();
    const loadingCell = !!s && s.x != null && !selectedReady;
    ...
    const reason = selectedCoolingDown ? "Map request paused · retrying shortly" : selectedFailed ? "Map data unavailable" : selectedLoading||loadingCell ? "Map data loading" : (this.pending ? "Working…" : shortCellReason(primary.reason || "Ready"));
    ...
    if (btn) { btn.disabled = !primary.enabled || this.pending || selectedLoading || selectedCoolingDown || selectedFailed || loadingCell; btn.textContent = this.pending ? "Working…" : primary.label; btn.classList.toggle("status-pill", !primary.enabled && !this.pending); btn.setAttribute("aria-label", `${primary.label} selected cell`); }
  ...
  nearestOverviewObjectAt(canvas, clientX, clientY, objects, radiusPx=18) { const ov=this.overview; if(!ov) return null; const t=this.overviewTransform(canvas,ov),r=canvas.getBoundingClientRect(),px=clientX-r.left,py=clientY-r.top; let best=null,bd=radiusPx*radiusPx; for(const o of objects||[]){const d=(t.ox+(+o.x)*t.scale-px)**2+(t.oy+(+o.y)*t.scale-py)**2;if(d<=bd){bd=d;best=o;}} return best; }
  ...
  renderWorldTargetSelection(target, selection) { if(!target || !selection) return; const x=Math.floor(selection.x), y=Math.floor(selection.y), dist=Math.abs(x-Math.floor(this.cx))+Math.abs(y-Math.floor(this.cy)), sectors=Math.ceil(dist/Math.max(1, this.overview?.world?.sector_size || this.state?.map?.sector_size || 40)); if(selection.rift){ target.innerHTML=`<b>Null Rift</b><br>Status: ${esc(selection.rift.status||"active")}<br>Position: ${x},${y}`; return; } if(selection.squad){ const sq=selection.squad, awaiting=sq.status==="awaiting_reinforcement", f=this.factionById(sq.faction_id); const detail=awaiting?`Reinforcement in: ${esc(this.reinforcementRemaining(sq))}<br>Return anchor: ${esc(sq.supply_x??"—")}, ${esc(sq.supply_y??"—")}<br>Expires in: ${esc(this.expirationRemaining(sq))}`:`${sq.status==="attacking_cell"?`Attack target: ${esc(sq.attack_target_x??"—")},${esc(sq.attack_target_y??"—")} · Progress: ${esc(sq.attack_progress??0)}${sq.attack_progress_required!=null?`/${esc(sq.attack_progress_required)}`:""}`:sq.status==="pressuring_capital"?`Capital pressure: ${esc(sq.attack_progress??0)}`:`Target: ${esc(sq.target_x??"—")},${esc(sq.target_y??"—")}`}`; target.innerHTML=`<b>Faction Squad</b><br>Faction: ${esc(f.name||"Faction")}<br>HP: ${esc(sq.hp)}/${esc(sq.max_hp)}<br>Position: ${x},${y}<br>Target: ${esc(sq.target_x??"—")},${esc(sq.target_y??"—")}<br>Supply: ${esc(sq.supply_x??"—")},${esc(sq.supply_y??"—")}<br>Status: ${esc(awaiting?"Awaiting reinforcement":sq.status)}<br>${detail}<br><button class="btn mini" data-open-tactical>Open Tactical Map</button>`; target.querySelector('[data-open-tactical]').onclick=()=>{ const modal=this.worldViewModal; if(modal) modal.remove(); this.worldViewModal=null; this.jumpToWorldPosition(+sq.x,+sq.y,10,{select:true}); this.refreshSquads(true); }; return; } const title=selection.hq?'Faction HQ':selection.capital?'Capital':esc(selection.controller||'Strategic target'); const status=selection.capitalStatus?`<br>Capital: ${esc(selection.capitalStatus)}`:''; const controlled=Number.isFinite(selection.controlledSectors)?`<br>Controlled sectors: ${selection.controlledSectors}`:''; const contested=Number.isFinite(selection.contestedSectors)?`<br>Contested sectors: ${selection.contestedSectors}`:''; target.innerHTML=`<b>${title}</b><br>Coordinates: ${x},${y}<br>Grid distance: ${dist} cells<br>Approx. sectors: ${sectors}${status}${controlled}${contested}<br><button class="btn mini" data-open-tactical>Open Tactical Map</button>`; target.querySelector('[data-open-tactical]').onclick=()=>{ const modal=this.worldViewModal; if(modal) modal.remove(); this.worldViewModal=null; this.jumpToWorldPosition(x,y,selection.hq?POLYWAR_VISUALS.baseZoom:10,{select:true}); }; }
  ...
  openWorldView() { if (this.worldViewModal && document.body.contains(this.worldViewModal)) { this.refreshSquadOverviewIfDue(true); this.renderOpenWorldView(); return; } if (this.worldViewModal) { this.worldViewModal.remove(); this.worldViewModal = null; } this.worldTargetSelection=null; this.refreshSquadOverviewIfDue(!this.overview || Date.now()-(this.lastSquadOverviewRefresh||0)>60000); const modal=document.createElement('div'); this.worldViewModal=modal; modal.className='polywar-world-view'; modal.innerHTML='<div class="world-view-panel"><header><b>World View</b><button class="btn mini" data-myhq>My HQ</button><button class="btn mini" data-close>Close</button></header><canvas class="polywar-world-canvas"></canvas><div class="world-view-legend" role="list" aria-label="World View legend"><span role="listitem">⬢ Faction HQ</span><span role="listitem">⬡ Faction Squad</span><span role="listitem">⬢ Capital</span><span role="listitem">◆ Null Rift</span><span role="listitem">▱ Current viewport</span><span role="listitem">■ Controlled territory</span></div><div class="world-target muted">Loading World View…</div></div>'; document.body.appendChild(modal); const canvas=modal.querySelector('canvas'); const target=modal.querySelector('.world-target'); modal.querySelector('[data-close]').onclick=()=>{ modal.remove(); if(this.worldViewModal===modal) this.worldViewModal=null; }; modal.querySelector('[data-myhq]').onclick=()=>{ const b=baseFor(currentState?.selected_faction?.id); if(b){ this.selectWorldTarget(target,{x:b.x,y:b.y,hq:{name:'My HQ'},capitalStatus:'Active'}); }}; canvas.addEventListener('pointerdown', e=>{ e.preventDefault(); e.stopPropagation(); if(!this.overview) return; const sq=this.nearestOverviewSquadAt(canvas,e.clientX,e.clientY,22); const rift=sq?null:this.nearestOverviewObjectAt(canvas,e.clientX,e.clientY,this.overview.major_objects,22); const h=(sq||rift)?null:this.nearestHqAt(canvas,e.clientX,e.clientY,22); const capital=(sq||rift||h)?null:this.nearestOverviewObjectAt(canvas,e.clientX,e.clientY,this.overview.capitals,22); const p=sq?{x:+sq.x,y:+sq.y,squad:sq}:rift?{x:+rift.x,y:+rift.y,rift}:h?{x:h.x,y:h.y,hq:h,capitalStatus:'Active'}:capital?{x:+capital.x,y:+capital.y,capital,capitalStatus:capital.is_under_siege?'Under siege':'Secure'}:this.overviewPointerToWorld(canvas,e.clientX,e.clientY,false); if(!p) return; if(!h){ const gx=Math.floor(p.x/(this.overview.overview_grid.world_per_column||1)), gy=Math.floor(p.y/(this.overview.overview_grid.world_per_row||1)); const cell=(this.overview.overview_grid.cells||[]).find(c=>c.grid_x===gx&&c.grid_y===gy); const f=this.factionById(cell?.controller_faction_id); p.controller=f.name||'Neutral territory'; p.controlledSectors=cell?.controlled_sector_count; p.contestedSectors=cell?.is_contested?1:0; } this.selectWorldTarget(target,p); }); requestAnimationFrame(()=>this.renderOpenWorldView()); }
...
if (typeof window !== "undefined") window.polywarChunkTestHooks = { PolyWarMap };
...
const polywarGovernanceUi = window.polywarGovernanceUi = window.polywarGovernanceUi || {
  ...
  render(data) {
    ...
    const isCommander = Boolean(data.current_user_is_leader) || Number(data.commander?.commander_user_id || data.commander?.user_id || 0) === Number(currentState?.player?.user_id || 0);
    ...
    const leaderboard=(data.leaderboard||[]).map(r=>`<li class="polywar-leaderboard-row ${r.is_current_user?'self':''}"><span>${r.rank===1?'👑':esc(r.rank)}</span>${renderLeaderAvatar(r,data.leader?.faction_color)}<b>${esc(r.display_name||'Player')}</b><small>${esc(r.faction_contribution||0)}</small></li>`).join('');
    const autoNote=data.automatic_leadership?'<p class="muted">Faction Leader is selected automatically by seasonal contribution among active members.</p>':'';
