...
class PolyWarMap {
  ...
  bind() {
    ...
    this.canvas.addEventListener("pointermove", e => { if (this.pointerStarts.size || e.pointerType === "touch") return; const p=this.screenToCell(e.offsetX,e.offsetY); const next=`${p.x},${p.y}`; if (next !== this.hovered) { this.hovered=next; this.requestDraw(); } }, { signal });
  ...
  ownerAt(x, y) {
    ...
    return Number(ch?.owners?.[y-ch.chunk_y*cs]?.[x-ch.chunk_x*cs] || 0);
  }
  drawOwnershipOverlay(ctx, owner, p, x, y) {
    if (!owner) return;
    const color=+owner===8 ? "#6d28d9" : (currentState.factions||[]).find(f=>+f.id===+owner)?.color || "#94a3b8", c=this.cell;
    ctx.save(); ctx.fillStyle=color; ctx.globalAlpha=POLYWAR_VISUALS.ownershipOpacity; ctx.fillRect(p.x,p.y,c,c);
    if (c >= POLYWAR_VISUALS.detailedCell) { ctx.globalAlpha=POLYWAR_VISUALS.ownershipPatternOpacity; ctx.fillStyle="#fff"; const step=Math.max(5,c*.28), offset=((x*3+y*5)%7); for(let k=-c;k<c*2;k+=step) ctx.fillRect(p.x+k+offset,p.y,1,c); }
    ctx.restore();
  }
  drawFactionBorders(ctx, owner, p, x, y) {
    if (!owner) return;
    const color=+owner===8 ? "#d8b4fe" : (currentState.factions||[]).find(f=>+f.id===+owner)?.color || "#e2e8f0", c=this.cell;
    const edges=[[0,-1,p.x,p.y,p.x+c,p.y],[1,0,p.x+c,p.y,p.x+c,p.y+c],[0,1,p.x,p.y+c,p.x+c,p.y+c],[-1,0,p.x,p.y,p.x,p.y+c]];
    ctx.save(); ctx.lineCap="square";
    for(const [dx,dy,x1,y1,x2,y2] of edges) { const neighbor=this.ownerAt(x+dx,y+dy); if (neighbor===+owner) continue; ctx.beginPath(); ctx.moveTo(x1,y1); ctx.lineTo(x2,y2); ctx.strokeStyle="rgba(5,10,22,.82)"; ctx.lineWidth=POLYWAR_VISUALS.borderThickness+2; ctx.stroke(); ctx.beginPath(); ctx.moveTo(x1,y1); ctx.lineTo(x2,y2); ctx.strokeStyle=color; ctx.globalAlpha=neighbor ? .92 : .62; ctx.lineWidth=POLYWAR_VISUALS.borderThickness; ctx.shadowColor=color; ctx.shadowBlur=this.cell>=10?POLYWAR_VISUALS.borderGlow:0; ctx.stroke(); }
    ctx.restore();
  }
  drawContestedOverlay(ctx, contest, p) {
    if (!contest) return;
    const c=this.cell, reduced=polywarReducedMotion(), phase=reduced ? .5 : (Date.now()%POLYWAR_VISUALS.contestedPulseMs)/POLYWAR_VISUALS.contestedPulseMs;
    const color=(currentState.factions||[]).find(f=>+f.id===+contest.contesting_faction_id)?.color||"#fbbf24";
    ctx.save(); ctx.beginPath(); ctx.rect(p.x,p.y,c,c); ctx.clip(); ctx.globalAlpha=.12+.08*Math.sin(phase*Math.PI*2); ctx.fillStyle=color; for(let k=-c;k<c*2;k+=POLYWAR_VISUALS.contestedStripe){ ctx.save(); ctx.translate(p.x+k,p.y); ctx.rotate(-Math.PI/4); ctx.fillRect(0,-c,2,c*3); ctx.restore(); } ctx.globalAlpha=1; ctx.strokeStyle="#fde68a"; ctx.lineWidth=2; ctx.shadowColor=color; ctx.shadowBlur=5; ctx.strokeRect(p.x+1,p.y+1,Math.max(2,c-2),Math.max(2,c-2)); ctx.shadowBlur=0; ctx.fillStyle=color; ctx.fillRect(p.x+3,p.y+c-5,Math.max(2,(c-6)*(contest.contest_progress/Math.max(1,contest.contest_required))),2); ctx.restore();
    if (!reduced && !document.hidden) setTimeout(()=>this.requestDraw(),160);
  }
  drawHoverCell(ctx) { if (!this.hovered || this.cell<8) return; const [x,y]=this.hovered.split(",").map(Number), p=this.cellToScreen(x,y); ctx.save(); ctx.fillStyle="rgba(226,245,255,.08)"; ctx.fillRect(p.x+1,p.y+1,this.cell-2,this.cell-2); ctx.strokeStyle="rgba(190,231,255,.48)"; ctx.strokeRect(p.x+1.5,p.y+1.5,this.cell-3,this.cell-3); ctx.restore(); }
  drawWorldPolish(ctx) { const g=ctx.createRadialGradient(this.w*.46,this.h*.38,Math.min(this.w,this.h)*.12,this.w*.5,this.h*.5,Math.max(this.w,this.h)*.7); g.addColorStop(0,"rgba(190,225,255,.025)"); g.addColorStop(.68,"rgba(7,13,27,0)"); g.addColorStop(1,"rgba(2,6,18,.25)"); ctx.fillStyle=g; ctx.fillRect(0,0,this.w,this.h); }
  ...
  drawSelectedCell(ctx) { if (!this.selected) return; const p=this.cellToScreen(this.selected.x,this.selected.y), reduced=polywarReducedMotion(), phase=reduced?.5:(Date.now()%POLYWAR_VISUALS.selectionPulseMs)/POLYWAR_VISUALS.selectionPulseMs, inset=2+Math.sin(phase*Math.PI*2)*.7; ctx.save(); ctx.fillStyle="rgba(53,166,255,.16)"; ctx.fillRect(p.x,p.y,this.cell,this.cell); ctx.strokeStyle="#b9efff"; ctx.shadowColor="#38bdf8"; ctx.shadowBlur=POLYWAR_VISUALS.selectionGlow; ctx.lineWidth=Math.max(2,Math.min(3,this.cell*.1)); ctx.strokeRect(p.x+inset,p.y+inset,Math.max(2,this.cell-inset*2),Math.max(2,this.cell-inset*2)); ctx.shadowBlur=0; ctx.strokeStyle="rgba(255,255,255,.9)"; ctx.lineWidth=1; const n=Math.min(6,this.cell*.22); for(const [sx,sy,dx,dy] of [[p.x,p.y,1,1],[p.x+this.cell,p.y,-1,1],[p.x,p.y+this.cell,1,-1],[p.x+this.cell,p.y+this.cell,-1,-1]]){ctx.beginPath();ctx.moveTo(sx+dx*n,sy);ctx.lineTo(sx,sy);ctx.lineTo(sx,sy+dy*n);ctx.stroke();} ctx.restore(); if(!reduced&&!document.hidden)setTimeout(()=>this.requestDraw(),180); }
