...
class PolyWarMap {
  ...
  updatePanel() {
    ...
    const attackCost = base == null ? null : base + 1, reinforceCost = 1;
    ...
    if (document.getElementById("cellSector")) document.getElementById("cellSector").textContent = s.x == null ? "—" : `${Math.floor(s.x/100)},${Math.floor(s.y/100)}`;
    ...
    const canCapture = !locked && fid && c.terrain && base != null && !c.owner && !this.pending && +currentState.energy.current_energy >= base;
    const canAttack = !locked && fid && c.terrain && base != null && c.owner && +c.owner !== +fid && this.isFrontline(s.x,s.y,fid) && !this.pending && +currentState.energy.current_energy >= attackCost;
    const canReinforce = !locked && fid && c.terrain && base != null && +c.owner === +fid && c.contest && +c.contest.contest_progress > 0 && !this.pending && +currentState.energy.current_energy >= reinforceCost;
    if (canAttack) actionMode = "attack"; else if (canReinforce) actionMode = "reinforce"; else if (canCapture) actionMode = "capture";
    document.getElementById("currentMode").textContent = actionMode;
    const btn = document.getElementById("captureBtn");
    const canMain = actionMode === "attack" ? canAttack : actionMode === "reinforce" ? canReinforce : canCapture;
    btn.disabled = !canMain; btn.textContent = this.pending ? "Working…" : (!fid ? "Choose faction" : actionMode === "attack" ? `Attack — ${attackCost} energy` : actionMode === "reinforce" ? `Reinforce — ${reinforceCost} energy` : "Capture");
  ...
  async capture() { if (!this.selected || this.pending) return; this.pending = true; this.updatePanel(); const d = await api("/api/polywar/action", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action_type: (actionMode === "attack" || actionMode === "reinforce") ? actionMode : "capture", x: this.selected.x, y: this.selected.y, idempotency_key: `cap-${Date.now()}-${Math.random().toString(16).slice(2)}` }) }); this.pending = false; if (!d.ok) { alert(d.error || "Action failed"); this.updatePanel(); return; } currentState.energy = d.energy; if (d.mine_hit) { this.blast = {x:this.selected.x,y:this.selected.y,t:Date.now()}; alert(`Mine hit — actions locked until ${d.locked_until || d.energy?.locked_until || "server unlock"} (${fmtTime(d.energy?.lock_seconds_remaining || 0)} remaining)`); } const cs = this.state.map.chunk_size; await this.ensureChunks(`${Math.floor(this.selected.x / cs)},${Math.floor(this.selected.y / cs)}`); updateEnergyUI(); if (d.outcome) alert(d.outcome); this.updatePanel(); this.requestDraw(); }
