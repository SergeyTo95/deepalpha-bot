...
class PolyWarMap {
  ...
  async bootstrapInitialLoad() { await this.ensureChunks(); if (this.destroyed) return; this.initialChunksReady = true; await Promise.allSettled([this.ensureSectors(), this.refreshCapitals(), this.refreshGovernance()]); this.loadOverview(); }
  ...
  centerOnBase(zoom = POLYWAR_VISUALS.baseZoom) { const b = baseFor(currentState?.selected_faction?.id); if (!b) return; this.cx = b.x; this.cy = b.y - 3; this.cell = Math.min(POLYWAR_VISUALS.maxCell, zoom); this.clamp(); this.ensureChunks(); this.ensureSectors(); this.requestDraw(); this.drawMinimap(); }
  jumpToWorldPosition(x, y, zoom = 12, options = {}) { x = Math.max(0, Math.min(this.state.map.width - 1, Math.floor(Number(x) || 0))); y = Math.max(0, Math.min(this.state.map.height - 1, Math.floor(Number(y) || 0))); this.cx = x; this.cy = y; this.cell = Math.max(POLYWAR_VISUALS.minCell, Math.min(POLYWAR_VISUALS.maxCell, Number(zoom) || 12)); this.clamp(); if (options.select !== false) this.selected = { x, y }; this.ensureSectors(); if (this.cell >= TACTICAL_MIN_CELL) { this.ensureChunks(`${Math.floor(x / this.state.map.chunk_size)},${Math.floor(y / this.state.map.chunk_size)}`); this.ensureChunks(); } this.updatePanel(); this.requestDraw(); this.drawMinimap(); }
...
function renderPolywarMenu(state){ const p=state.player||{}, e=state.energy||{}, season=state.season||{}, selected=state.selected_faction, needsJoin=!selected; return `<div class="polywar-menu-backdrop" id="polywarMenuBackdrop" data-polywar-menu-close="backdrop"></div><aside class="polywar-menu-sheet glass" id="polywarMenuSheet" role="dialog" aria-modal="true" aria-labelledby="polywarMenuTitle"><header class="polywar-menu-head"><div><p class="eyebrow">Command menu</p><h2 id="polywarMenuTitle">PolyWar status</h2></div><button class="btn mini" id="polywarMenuClose" data-polywar-menu-close="button" aria-label="Close PolyWar menu">Close</button></header><div class="polywar-menu-scroll"><section class="polywar-menu-section"><h3>Overview</h3><div class="polywar-menu-grid"><div class="polywar-info-card"><span>Season</span><b>${esc(season.name||"Active Season")}</b><small>${esc(season.starts_at||"—")} → ${esc(season.ends_at||"—")}</small></div><div class="polywar-info-card"><span>Energy</span><b id="energyValue">${esc(e.current_energy)}/${esc(e.max_energy)}</b><small>Next charge: <span id="energyCountdown">${fmtTime(e.seconds_until_next_energy)}</span> · ${esc(e.recharge_minutes)} min</small><small>Status: <b id="lockStatus">${e.is_locked?"Mine locked":"Active"}</b></small></div><div class="polywar-info-card ${selected?"confirm":""}"><span>Faction</span><b>${selected?`${factionDot(selected)}${esc(selected.name)}`:"Not selected"}</b><small>${selected?"Faction locked for this season.":"Choose a faction to capture cells."}</small></div></div>${needsJoin?`<div class="factions polywar-menu-factions">${(state.factions||[]).map(f=>`<button class="faction" data-faction="${esc(f.id)}">${factionDot(f)}${esc(f.name)}<small>${esc(f.description)}</small></button>`).join("")}</div>`:""}</section><section class="polywar-menu-section polywar-world-hud" id="polywarWorldHud"><h3>World HUD</h3>${renderWorldHud(state)}</section><section class="polywar-menu-section polywar-governance-panel" id="polywarGovernancePanel" data-polywar-governance><h3>Governance</h3></section><section class="polywar-menu-section"><h3>Ranking</h3><div class="polywar-menu-grid" id="factionStats"><div class="polywar-info-card"><span>Season Points</span><b>${esc(p.season_spendable_points||0)}</b></div><div class="polywar-info-card"><span>Faction Contribution</span><b>${esc(p.faction_contribution||0)}</b></div></div><div id="factionRanking" class="polywar-ranking-list"></div></section><section class="polywar-menu-section polywar-results-panel" id="polywarResultsPanel"><h3>Season Results</h3>${renderResultsPanel(state)}</section><section class="polywar-menu-section"><h3>Latest events</h3><div id="latestEvents"></div></section></div></aside>`; }
function openPolywarMenu(){ const layer=document.getElementById('polywarMenuLayer'); if(!layer||layer.dataset.open==='true') return; layer.innerHTML=renderPolywarMenu(currentState||{}); layer.dataset.open='true'; document.body.classList.add('polywar-menu-open'); updateFactionStats(); updateFactionRanking(); updateLatestEvents(); polywarGovernanceUi.render(polywarGovernanceUi.lastData||{}); startEnergyTimers(); startWorldCountdownTimer(); }
function closePolywarMenu(){ const layer=document.getElementById('polywarMenuLayer'); if(!layer) return; layer.innerHTML=''; layer.dataset.open='false'; document.body.classList.remove('polywar-menu-open'); startEnergyTimers(); startWorldCountdownTimer(); }
...
window.addEventListener("pagehide", () => { clearTimers(); closePolywarMenu(); map?.destroy(); map = null; });
