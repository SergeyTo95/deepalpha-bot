...
class PolyWarMap {
  constructor(state) {
    ...
    this.overview = null; this.overviewError = null;
  ...
  bind() {
    ...
    document.getElementById("polywarMinimapToggle")?.addEventListener("click", () => document.querySelector(".polywar-minimap")?.classList.toggle("is-collapsed"), { signal });
  ...
  async loadOverview() { const seq = ++polywarOverviewSeq; try { const data = await api('/api/polywar/world/overview'); if (seq !== polywarOverviewSeq || this.destroyed) return; if (!data?.ok || Number(data.season_id) !== Number(currentState?.season?.id)) return; this.overview = data; this.overviewError = null; this.drawMinimap(); } catch (e) { this.overviewError = e?.message || 'overview_failed'; this.drawMinimap(); } }
  drawOverviewCanvas(canvas) { const ov=this.overview, ctx=canvas?.getContext('2d'); if(!ov||!ctx) return; const dpr=Math.min(2,window.devicePixelRatio||1), r=canvas.getBoundingClientRect(); canvas.width=Math.max(1,Math.floor(r.width*dpr)); canvas.height=Math.max(1,Math.floor(r.height*dpr)); ctx.setTransform(dpr,0,0,dpr,0,0); ctx.clearRect(0,0,r.width,r.height); const pad=10, scale=Math.min((r.width-pad*2)/ov.world.width,(r.height-pad*2)/ov.world.height), ox=(r.width-ov.world.width*scale)/2, oy=(r.height-ov.world.height*scale)/2, factions=ov.factions||[]; ctx.fillStyle='rgba(8,14,30,.95)'; ctx.fillRect(0,0,r.width,r.height); for(const cell of ov.overview_grid.cells||[]){ const f=factions.find(q=>+q.id===+cell.controller_faction_id); if(!f) continue; ctx.globalAlpha=Math.max(.18,Math.min(.75,(cell.dominance_percent||30)/100)); ctx.fillStyle=f.color; ctx.fillRect(ox+cell.grid_x*ov.overview_grid.world_per_column*scale,oy+cell.grid_y*ov.overview_grid.world_per_row*scale,Math.ceil(ov.overview_grid.world_per_column*scale)+1,Math.ceil(ov.overview_grid.world_per_row*scale)+1); ctx.globalAlpha=1; if(cell.is_contested){ ctx.strokeStyle='rgba(255,255,255,.55)'; ctx.strokeRect(ox+cell.grid_x*ov.overview_grid.world_per_column*scale,oy+cell.grid_y*ov.overview_grid.world_per_row*scale,Math.ceil(ov.overview_grid.world_per_column*scale),Math.ceil(ov.overview_grid.world_per_row*scale)); }} for(const h of ov.hq||[]){ const x=ox+h.x*scale,y=oy+h.y*scale; ctx.fillStyle=h.color||'#fff'; ctx.beginPath(); ctx.arc(x,y,5,0,Math.PI*2); ctx.fill(); ctx.strokeStyle='#fff'; ctx.stroke(); } for(const c of ov.capitals||[]){ const x=ox+c.x*scale,y=oy+c.y*scale; ctx.fillStyle='#fff'; ctx.fillRect(x-2,y-2,4,4); } const a=this.screenToCell(0,0), b=this.screenToCell(this.w,this.h); ctx.strokeStyle='#00e5ff'; ctx.lineWidth=1.5; ctx.strokeRect(ox+Math.max(0,Math.min(a.x,b.x))*scale,oy+Math.max(0,Math.min(a.y,b.y))*scale,Math.abs(b.x-a.x)*scale,Math.abs(b.y-a.y)*scale); }
  drawMinimap() { if(!this.minimapCanvas) return; this.drawOverviewCanvas(this.minimapCanvas); }
  handleMinimapPointer(e) { e.preventDefault(); e.stopPropagation(); this.minimapCanvas.setPointerCapture?.(e.pointerId); const ov=this.overview; if(!ov) { this.loadOverview(); return; } const r=this.minimapCanvas.getBoundingClientRect(), wx=Math.max(0,Math.min(ov.world.width-1,((e.clientX-r.left)/r.width)*ov.world.width)), wy=Math.max(0,Math.min(ov.world.height-1,((e.clientY-r.top)/r.height)*ov.world.height)); let nearest=null, nd=1e18; for(const h of ov.hq||[]){ const d=(h.x-wx)**2+(h.y-wy)**2; if(d<nd){nd=d; nearest=h;} } const snap=Math.max(ov.world.width,ov.world.height)/10; if(nearest && Math.sqrt(nd)<snap) this.jumpToWorldPosition(nearest.x, nearest.y, POLYWAR_VISUALS.baseZoom, {select:true}); else this.jumpToWorldPosition(wx, wy, 10, {select:true}); }
  openWorldView() { if(!this.overview) this.loadOverview(); const modal=document.createElement('div'); modal.className='polywar-world-view'; modal.innerHTML='<div class="world-view-panel"><header><b>World View</b><button class="btn mini" data-myhq>My HQ</button><button class="btn mini" data-close>Close</button></header><canvas class="polywar-world-canvas"></canvas><div class="world-target muted">Tap a HQ, capital, object or territory to open tactical view.</div></div>'; document.body.appendChild(modal); const canvas=modal.querySelector('canvas'); const render=()=>this.drawOverviewCanvas(canvas); requestAnimationFrame(render); modal.querySelector('[data-close]').onclick=()=>modal.remove(); modal.querySelector('[data-myhq]').onclick=()=>{ const b=baseFor(currentState?.selected_faction?.id); if(b){ modal.remove(); this.jumpToWorldPosition(b.x,b.y,POLYWAR_VISUALS.baseZoom,{select:true}); }}; canvas.addEventListener('pointerdown', e=>{ e.preventDefault(); e.stopPropagation(); const ov=this.overview; if(!ov) return; const r=canvas.getBoundingClientRect(), wx=((e.clientX-r.left)/r.width)*ov.world.width, wy=((e.clientY-r.top)/r.height)*ov.world.height; modal.remove(); this.jumpToWorldPosition(wx,wy,10,{select:true}); }); }
