...
class PolyWarMap {
  constructor(state) {
    ...
    this.squads = []; this.squadPressure = []; this.squadSeq = 0; this.squadAbort = null; this.squadRefreshTimer = null; this.squadDebounceTimer = null; this.squadAnimations = new Map(); this.squadSupportEnergyCost = 1;
  ...
  centerOnBase(zoom = POLYWAR_VISUALS.baseZoom) { const b = baseFor(currentState?.selected_faction?.id); if (!b) return; this.cx = b.x; this.cy = b.y - 3; this.cell = Math.min(POLYWAR_VISUALS.maxCell, zoom); this.clamp(); this.ensureChunks(); this.ensureSectors(); this.refreshSquads(true); this.requestDraw(); this.drawMinimap(); }
  jumpToWorldPosition(x, y, zoom = 12, options = {}) { x = Math.max(0, Math.min(this.state.map.width - 1, Math.floor(Number(x) || 0))); y = Math.max(0, Math.min(this.state.map.height - 1, Math.floor(Number(y) || 0))); this.cx = x; this.cy = y; this.cell = Math.max(POLYWAR_VISUALS.minCell, Math.min(POLYWAR_VISUALS.maxCell, Number(zoom) || 12)); this.clamp(); if (options.select !== false) this.selected = { x, y }; this.ensureSectors(); this.refreshSquads(true); if (this.cell >= TACTICAL_MIN_CELL) { this.ensureChunks(`${Math.floor(x / this.state.map.chunk_size)},${Math.floor(y / this.state.map.chunk_size)}`); this.ensureChunks(); } this.updatePanel(); this.requestDraw(); this.drawMinimap(); }
  ...
  updatePanel() {
    ...
    const squad = this.squadAt(s.x, s.y), pressures = this.pressureAt(s.x, s.y);
    const squadHtml = squad ? `<span class="sheet-extra-detail"><b>Faction Vanguard</b> HP: ${esc(squad.hp)}/${esc(squad.max_hp)} · Status: ${esc(squad.status)} · Target: ${esc(squad.target_x ?? "—")},${esc(squad.target_y ?? "—")} ${+squad.faction_id===+(currentState?.selected_faction?.id||0) ? `<button class="secondary-action-pill" data-polywar-support-squad="${esc(squad.id)}">Support · ${esc(this.squadSupportEnergyCost || 1)} ⚡</button>` : ""}</span>` : "";
  ...
  drawSquads(ctx){ const now=performance.now(); let anim=false; for(const sq of this.squads||[]){ let x=+sq.x,y=+sq.y; const a=this.squadAnimations.get(String(sq.id)); if(a && !polywarReducedMotion() && !document.hidden){ const t=Math.min(1,(now-a.start)/a.duration); x=a.fromX+(a.toX-a.fromX)*t; y=a.fromY+(a.toY-a.fromY)*t; if(t<1) anim=true; else this.squadAnimations.delete(String(sq.id)); } const f=this.factionById(sq.faction_id); const p=this.cellToScreen(x,y), cx=p.x+this.cell/2, cy=p.y+this.cell/2, r=Math.max(3,Math.min(7,this.cell*.18)); ctx.fillStyle=darkenFactionColor(f.color,.38); ctx.strokeStyle=contrastBorderForFaction(f.color); ctx.lineWidth=1.5; for(const [dx,dy] of [[0,-r],[-r,r],[r,r]]){ ctx.beginPath(); ctx.arc(cx+dx,cy+dy,r,0,Math.PI*2); ctx.fill(); ctx.stroke(); } ctx.fillStyle=contrastBorderForFaction(f.color); ctx.font=`${Math.max(8,Math.min(12,this.cell*.32))}px sans-serif`; ctx.textAlign="center"; ctx.fillText(this.factionLabel(f), cx, cy-r-3); const hp=Math.max(0,Math.min(1,(+sq.hp||0)/(+sq.max_hp||1))); ctx.fillStyle="rgba(15,23,42,.85)"; ctx.fillRect(p.x+2,p.y+this.cell-5,this.cell-4,3); ctx.fillStyle="#22c55e"; ctx.fillRect(p.x+2,p.y+this.cell-5,(this.cell-4)*hp,3); } if(anim) this.requestDraw(); }
  async supportSelectedSquad(id){ const key=`${currentState?.season?.id}:support_squad:${id}`; const idem=polywarActionKeys.get(key)||`${key}:${Date.now()}:${Math.random().toString(16).slice(2)}`; polywarActionKeys.set(key, idem); const d=await api(`/api/polywar/squads/${id}/support`, {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({idempotency_key:idem})}); if(!d.ok){ toast(d.error||"Support failed", true); return d; } polywarActionKeys.delete(key); currentState.energy=d.energy||currentState.energy; toast("Squad supported"); await this.refreshSquads(true); updateEnergyUI(); this.updatePanel(); return d; }
  ...
  drawStrategicMarker(ctx, x, y, color, label, { player=false, capital=false, siege=0, border='#ffffff' } = {}) { const r=player?8:capital?7:6; ctx.save(); if(player){ ctx.shadowColor=color; ctx.shadowBlur=10; } ctx.beginPath(); for(let i=0;i<6;i++){ const a=Math.PI/6+i*Math.PI/3; const px=x+Math.cos(a)*r, py=y+Math.sin(a)*r; i?ctx.lineTo(px,py):ctx.moveTo(px,py); } ctx.closePath(); ctx.fillStyle=color||'#e5e7eb'; ctx.fill(); ctx.shadowBlur=0; ctx.lineWidth=player?2.4:1.4; ctx.strokeStyle=border; ctx.stroke(); ctx.fillStyle=(color||'').toLowerCase()==='#111827'?'#fff':'#08111f'; ctx.font=`700 ${capital?8:7}px system-ui`; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText(label||'•',x,y+.4); if(siege){ ctx.beginPath(); ctx.strokeStyle='rgba(250,204,21,.95)'; ctx.lineWidth=2; ctx.arc(x,y,r+4,-Math.PI/2,-Math.PI/2+Math.PI*2*Math.max(0,Math.min(1,siege)),false); ctx.stroke(); } ctx.restore(); }
  drawViewportRect(ctx, t, scale, ox, oy) { const a=this.screenToCell(0,0), b=this.screenToCell(this.w,this.h); const x1=Math.max(0,Math.min(a.x,b.x)), y1=Math.max(0,Math.min(a.y,b.y)), x2=Math.min(this.state.map.width,Math.max(a.x,b.x)), y2=Math.min(this.state.map.height,Math.max(a.y,b.y)); let rx=ox+x1*scale, ry=oy+y1*scale, rw=Math.max(5,(x2-x1)*scale), rh=Math.max(5,(y2-y1)*scale); rx=Math.max(ox,Math.min(ox+t.renderedWidth-rw,rx)); ry=Math.max(oy,Math.min(oy+t.renderedHeight-rh,ry)); ctx.fillStyle='rgba(34,211,238,.14)'; ctx.strokeStyle='rgba(240,253,255,.96)'; ctx.lineWidth=1.5; ctx.fillRect(rx,ry,rw,rh); ctx.strokeRect(rx,ry,rw,rh); }
  drawOverviewCanvas(canvas) { const ov=this.overview, ctx=canvas?.getContext('2d'); if(!ov||!ctx) return; const dpr=Math.min(2,window.devicePixelRatio||1), t=this.overviewTransform(canvas, ov); canvas.width=Math.max(1,Math.floor(t.cssWidth*dpr)); canvas.height=Math.max(1,Math.floor(t.cssHeight*dpr)); ctx.setTransform(dpr,0,0,dpr,0,0); ctx.clearRect(0,0,t.cssWidth,t.cssHeight); const {scale,ox,oy}=t, factions=ov.factions||[]; const grad=ctx.createLinearGradient(0,0,t.cssWidth,t.cssHeight); grad.addColorStop(0,'rgba(20,34,56,.96)'); grad.addColorStop(1,'rgba(7,16,31,.96)'); ctx.fillStyle=grad; ctx.fillRect(0,0,t.cssWidth,t.cssHeight); ctx.fillStyle='rgba(148,163,184,.08)'; ctx.fillRect(ox,oy,t.renderedWidth,t.renderedHeight); const wpcol=ov.overview_grid.world_per_column, wprow=ov.overview_grid.world_per_row; for(const cell of ov.overview_grid.cells||[]){ const f=factions.find(q=>+q.id===+cell.controller_faction_id); if(!f) continue; const x=ox+cell.grid_x*wpcol*scale,y=oy+cell.grid_y*wprow*scale,w=Math.ceil(wpcol*scale)+1,h=Math.ceil(wprow*scale)+1; ctx.globalAlpha=Math.max(.22,Math.min(.82,(cell.dominance_percent||35)/100)); ctx.fillStyle=f.color; ctx.fillRect(x,y,w,h); ctx.globalAlpha=1; if(cell.is_contested){ ctx.strokeStyle='rgba(255,255,255,.75)'; ctx.lineWidth=1; ctx.strokeRect(x+.5,y+.5,Math.max(1,w-1),Math.max(1,h-1)); ctx.beginPath(); ctx.strokeStyle='rgba(15,23,42,.55)'; for(let hx=x-w; hx<x+w; hx+=6){ ctx.moveTo(hx,y+h); ctx.lineTo(hx+h,y); } ctx.stroke(); }} ctx.strokeStyle='rgba(148,163,184,.10)'; ctx.lineWidth=1; const step=Math.max(8, ov.world.sector_size*scale*4); for(let gx=ox;gx<=ox+t.renderedWidth;gx+=step){ctx.beginPath();ctx.moveTo(gx,oy);ctx.lineTo(gx,oy+t.renderedHeight);ctx.stroke();} for(let gy=oy;gy<=oy+t.renderedHeight;gy+=step){ctx.beginPath();ctx.moveTo(ox,gy);ctx.lineTo(ox+t.renderedWidth,gy);ctx.stroke();} for(const z of ov.starting_zones||[]){ const f=this.factionById(z.faction_id); ctx.strokeStyle=f.color||'rgba(255,255,255,.5)'; ctx.globalAlpha=.8; ctx.setLineDash([3,2]); ctx.strokeRect(ox+z.min_x*scale,oy+z.min_y*scale,Math.max(2,(z.max_x-z.min_x)*scale),Math.max(2,(z.max_y-z.min_y)*scale)); ctx.setLineDash([]); ctx.globalAlpha=1; } for(const p of ov.squad_pressure_bins||[]){ const f=this.factionById(p.faction_id); const x=ox+p.grid_x*wpcol*scale,y=oy+p.grid_y*wprow*scale,w=Math.ceil(wpcol*scale)+1,h=Math.ceil(wprow*scale)+1; ctx.globalAlpha=Math.max(.25,Math.min(.65,(p.pressure||0)/100*.65)); ctx.fillStyle=darkenFactionColor(f.color,.24); ctx.fillRect(x,y,w,h); ctx.globalAlpha=1; if(p.is_contested){ ctx.strokeStyle=contrastBorderForFaction(f.color); ctx.strokeRect(x,y,w,h); }} for(const sq of ov.squads||[]){ const f=this.factionById(sq.faction_id); this.drawStrategicMarker(ctx,ox+sq.x*scale,oy+sq.y*scale,darkenFactionColor(f.color,.38),this.factionLabel(f),{border:contrastBorderForFaction(f.color)}); } for(const c of ov.capitals||[]){ const orig=this.factionById(c.original_faction_id), ctrl=this.factionById(c.controller_faction_id); this.drawStrategicMarker(ctx,ox+c.x*scale,oy+c.y*scale,ctrl.color||orig.color||'#fff',this.factionLabel(orig),{capital:true,border:orig.color||'#fff',siege:(c.siege_progress||0)/1000}); } for(const h of ov.hq||[]){ const f=this.factionById(h.faction_id); this.drawStrategicMarker(ctx,ox+h.x*scale,oy+h.y*scale,h.color||f.color||'#fff',this.factionLabel(f),{player:+h.faction_id===+(currentState?.selected_faction?.id||0),border:'#f8fafc'}); } for(const o of ov.major_objects||[]){ const x=ox+o.x*scale,y=oy+o.y*scale; ctx.fillStyle=o.status==='active'?'#f43f5e':'#94a3b8'; ctx.beginPath(); ctx.moveTo(x,y-5); ctx.lineTo(x+5,y); ctx.lineTo(x,y+5); ctx.lineTo(x-5,y); ctx.closePath(); ctx.fill(); } this.drawViewportRect(ctx,t,scale,ox,oy); }
  ...
  renderWorldTargetSelection(target, selection) { if(!target || !selection) return; const x=Math.floor(selection.x), y=Math.floor(selection.y), dist=Math.abs(x-Math.floor(this.cx))+Math.abs(y-Math.floor(this.cy)), sectors=Math.ceil(dist/Math.max(1, this.overview?.world?.sector_size || this.state?.map?.sector_size || 40)); const title=selection.hq?esc(selection.hq.name||'HQ'):selection.capital?esc(selection.capital.name||'Capital'):esc(selection.controller||'Strategic target'); const status=selection.capitalStatus?`<br>Capital: ${esc(selection.capitalStatus)}`:''; const controlled=Number.isFinite(selection.controlledSectors)?`<br>Controlled sectors: ${selection.controlledSectors}`:''; const contested=Number.isFinite(selection.contestedSectors)?`<br>Contested sectors: ${selection.contestedSectors}`:''; target.innerHTML=`<b>${title}</b><br>Coordinates: ${x},${y}<br>Grid distance: ${dist} cells<br>Approx. sectors: ${sectors}${status}${controlled}${contested}<br><button class="btn mini" data-open-tactical>Open Tactical Map</button>`; target.querySelector('[data-open-tactical]').onclick=()=>{ const modal=this.worldViewModal; if(modal) modal.remove(); this.worldViewModal=null; this.jumpToWorldPosition(x,y,selection.hq?POLYWAR_VISUALS.baseZoom:10,{select:true}); }; }
