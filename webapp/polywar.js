...
class PolyWarMap {
  ...
  async refreshWorld() { const seq = ++this.worldSeq; const d = await api("/api/polywar/world"); if (seq !== this.worldSeq || this.destroyed) return {ok:false, stale:true}; if (d.ok && d.world) { currentState.world = d.world; this.requestDraw(); } return d; }
  ...
  async specialAction(action_type) { if (!this.selected || this.pending) return; this.pending = true; this.updatePanel(); const d = await api("/api/polywar/action", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({action_type, x:this.selected.x, y:this.selected.y, idempotency_key:`${action_type}-${Date.now()}-${Math.random().toString(16).slice(2)}`})}); this.pending = false; if(!d.ok){ alert(d.error || "Action failed"); this.updatePanel(); return d; } currentState.energy=d.energy||currentState.energy; const cs=this.state.map.chunk_size; await this.ensureChunks(`${Math.floor(this.selected.x/cs)},${Math.floor(this.selected.y/cs)}`); await this.refreshWorld(); updateEnergyUI(); this.updatePanel(); this.requestDraw(); return d; }
