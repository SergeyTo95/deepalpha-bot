...
const I18N = {
  en: {
    subtitle: 'AI-powered prediction market intelligence', guestPrompt: 'Sign in to access your DeepAlpha dashboard',
    ...
    cashier: 'Cashier', cashierDesc: 'Buy tokens and manage payments', openCashier: 'Open Cashier', actions: 'Quick actions', logout: 'Logout', active: 'Active'
  },
  ru: {
    subtitle: 'AI-анализ рынков прогнозов', guestPrompt: 'Войдите, чтобы открыть личный кабинет DeepAlpha',
    ...
    cashier: 'Касса', cashierDesc: 'Покупка токенов и управление оплатой', openCashier: 'Открыть кассу', actions: 'Быстрые действия', logout: 'Выйти', active: 'Активна'
  }
};
...
function setSubtitle(lang){ const el=document.getElementById('heroSubtitle'); if(el) el.textContent=I18N[lang].subtitle; }

function renderGuest(lang){
  const t=I18N[lang]; setSubtitle(lang);
  ...
  document.getElementById('tgBtn').onclick=async()=>{const ok=await telegramAuthIfAvailable(); if(ok) return init(); alert('Telegram auth unavailable. Open this page from Telegram WebApp.');};
