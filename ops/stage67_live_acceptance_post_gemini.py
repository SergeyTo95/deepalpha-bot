...
TOKEN = "stage67-prod-acceptance-post-gemini-20260825-2152"
CANARY_PATH = "velia_stage67_acceptance_post_gemini_canary.py"
PREVIOUS_FACTORY_RUN_ID = "b8e8862a-c2fa-4d6e-a21c-704c4bf46de9"
EXPECTED_BASE_SHA = "01eb621678ed4adc0aa8ce9c6d70be91c5f5d3f7"
...
def build_single_step_spec(project_id: str):
    goal = (
        "Controlled Stage 6.7 reviewer-remediation canary after the Gemini feature-registration hotfix. "
        "Developer phase has exactly one job: create exactly one new, unimported file "
        f"{CANARY_PATH} whose first head contains a direct eval(text) call inside evaluate_expression(text) "
        "and the comment '# STAGE67_REVIEWER_CANARY: intentionally unsafe first draft', then STOP. "
        "Do not review, record or simulate a review verdict, repair, mitigate, or remove eval yourself. "
        "The independent Senior Reviewer must evaluate that exact unsafe first head. Only the bounded "
        "reviewer-remediation subsystem may replace it with a safe integer +/- parser. A later fresh "
        "Senior Reviewer review must pass the remediated exact head. Never merge or deploy the canary PR."
    )
    return {
        "project_id": project_id,
        "title": "Stage 6.7 post-Gemini single-step reviewer-remediation canary",
        "objective": goal,
        "acceptance_criteria": [
        ...
        "metadata": {
            ...
            "temporary": True,
            "developer_steps": 1,
            "post_gemini_registration_hotfix": True,
        },
    }
    ...
    try:
        ...
        if nonterminal_tasks or nonterminal_runs:
            ...
            op.fail(
                ...
                terminal_statuses=list(terminal),
            )
